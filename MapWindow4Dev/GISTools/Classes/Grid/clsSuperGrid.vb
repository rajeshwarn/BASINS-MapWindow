...
Public Class SuperGrid
        ...
        Try
            ...
            Do Until EOF(FileNum)
                ...
                If InStr(OneLine, "nodata_value") > 0 Then
                    Integer.TryParse(OneLine.Replace("nodata_value", "").Replace(" ", "").Replace(vbTab, ""), FLTData.NODATA_value)
    ...
    Public Function grd2asc(ByRef sGridfile As String, ByRef sAscfile As String, Optional ByRef errMsg As String = Nothing) As Boolean
        ...
        Try
            ...
            'The file is closed in the Finally block now
    ...
    Private Function ImportSURFERFormat(ByVal InFile As String, ByRef Callback As MapWinGIS.ICallback) As Boolean
        'Read the .DEM data stream and process it
        'Dan Ames April 2003
        Dim tempPath As String = System.IO.Path.ChangeExtension(System.IO.Path.GetTempFileName(), "")
    ...
    Private Sub MakeGrid(ByVal FLTData As sFLTData, ByVal InitialFile As String, ByRef Callback As MapWinGIS.ICallback)
        ...
        MyBase.Close()
        MyBase.CreateNew("", h, MapWinGIS.GridDataType.ShortDataType, -1, True, MapWinGIS.GridFileType.Binary)
    ...
    Private Function ReadDEMData(ByVal St As Stream, ByRef Callback As MapWinGIS.ICallback) As sDEMData
        ...
        Try
            'get header data
            DEMData.Notes = GetChunk(St, 1, 144)
            DEMData.HorizUnits = Int(GetChunk(St, 529, 6))
            DEMData.VertUnits = Int(GetChunk(St, 535, 6))
            ReDim DEMData.Vertices(3)
            For i = 0 To 3
                DEMData.Vertices(i).X = Fdbl(GetChunk(St, 547 + (48 * i), 24))
                DEMData.Vertices(i).Y = Fdbl(GetChunk(St, 571 + (48 * i), 24))
            Next
            DEMData.Min = Fdbl(GetChunk(St, 739, 24))
            DEMData.Max = Fdbl(GetChunk(St, 763, 24))
            DEMData.NumCols = Int(GetChunk(St, 859, 6))
            ...
            For i = 0 To DEMData.NumCols - 1
                ...
                numElevs = Int(GetChunk(St, Off2 + 13, 6))
                DEMData.NumElevs(i) = numElevs
                DEMData.ColStarts(i).X = Fdbl(GetChunk(St, Off2 + 25, 24))
                DEMData.ColStarts(i).Y = Fdbl(GetChunk(St, Off2 + 49, 24))
            ...
            Return DEMData
        Catch ex As System.Exception
            MapWinUtility.Logger.Msg("Error in ProcessData: " & ex.Message)
        End Try
        Return Nothing
    End Function
    ...
    Public Overloads Function Open(ByVal Filename As String, _
                          Optional ByVal DataType As MapWinGIS.GridDataType = MapWinGIS.GridDataType.UnknownDataType, _
                          Optional ByVal InRam As Boolean = True, _
                          Optional ByVal FileType As MapWinGIS.GridFileType = MapWinGIS.GridFileType.UseExtension, _
                          Optional ByVal cBack As MapWinGIS.ICallback = Nothing) As Boolean
