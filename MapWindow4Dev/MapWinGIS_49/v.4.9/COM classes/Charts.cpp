...
CPLXMLNode* CCharts::SerializeCore(CString ElementName)
{
	...
	if (_bars.size() > 0)
	{
		...
		if (psFields)
		{
			for (unsigned int i = 0; i < _bars.size(); i++)
			{
				...
				str = OLE2CA(name);
				SysFreeString(name);
				CPLCreateXMLAttributeAndValue(psNode, "Name", str);
			
				// color
				OLE_COLOR color;
				_bars[i]->get_Color(&color);
				CPLCreateXMLAttributeAndValue(psNode, "Color", CPLString().Printf("%d", color));
				
				// index
				long index;
				_bars[i]->get_Index(&index);
				CPLCreateXMLAttributeAndValue(psNode, "Index", CPLString().Printf("%d", index));
			}
		}
	}
	...
	if (_options.avoidCollisions != opt.avoidCollisions)
		CPLCreateXMLAttributeAndValue(psTree, "AvoidCollisions", CPLString().Printf("%d", (int)_options.avoidCollisions));

	if (_options.barHeight != opt.barHeight)
		CPLCreateXMLAttributeAndValue(psTree, "BarHeight", CPLString().Printf("%d", _options.barHeight));
	
	if (_options.barWidth != opt.barWidth)
		CPLCreateXMLAttributeAndValue(psTree, "BarWidth", CPLString().Printf("%d", _options.barWidth));

	if (_options.chartType != opt.chartType)
		CPLCreateXMLAttributeAndValue(psTree, "ChartType", CPLString().Printf("%d", (int)_options.chartType));

	if (_options.lineColor != opt.lineColor)
		CPLCreateXMLAttributeAndValue(psTree, "LineColor", CPLString().Printf("%d", _options.lineColor));

	if (_options.normalizationField != opt.normalizationField)
		CPLCreateXMLAttributeAndValue(psTree, "NormalizationField", CPLString().Printf("%d", (int)_options.normalizationField));

	if (_options.normalizationField != opt.normalizationField)
		CPLCreateXMLAttributeAndValue(psTree, "Radius", CPLString().Printf("%d", (int)_options.radius));

	if (_options.radius2 != opt.radius2)
		CPLCreateXMLAttributeAndValue(psTree, "Radius2", CPLString().Printf("%d", (int)_options.radius2));

	if (_options.rotation != opt.rotation)
		CPLCreateXMLAttributeAndValue(psTree, "Rotation", CPLString().Printf("%f", _options.rotation));

	if (_options.sizeField != opt.sizeField)
		CPLCreateXMLAttributeAndValue(psTree, "SizeField", CPLString().Printf("%d", _options.sizeField));

	if (_options.thickness != opt.thickness)
		CPLCreateXMLAttributeAndValue(psTree, "Thickness", CPLString().Printf("%f", _options.thickness));

	if (_options.tilt != opt.tilt)
		CPLCreateXMLAttributeAndValue(psTree, "Tilt", CPLString().Printf("%f", _options.tilt));
	
	if (_options.transparency != opt.transparency)
		CPLCreateXMLAttributeAndValue(psTree, "Transparency", CPLString().Printf("%d", _options.transparency));
	
	if (_options.use3Dmode != opt.use3Dmode)
		CPLCreateXMLAttributeAndValue(psTree, "Use3Dmode", CPLString().Printf("%d", (int)_options.use3Dmode));
	
	if (_options.useVariableRadius != opt.useVariableRadius)
		CPLCreateXMLAttributeAndValue(psTree, "UseVariableRadius", CPLString().Printf("%d", (int)_options.useVariableRadius));
	
	if (_options.valuesFontBold != opt.valuesFontBold)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFontBold", CPLString().Printf("%d", (int)_options.valuesFontBold));
	
	if (_options.valuesFontColor != opt.valuesFontColor)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFontColor", CPLString().Printf("%d", _options.valuesFontColor));
	
	if (_options.valuesFontItalic != opt.valuesFontItalic)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFontItalic", CPLString().Printf("%d", (int)_options.valuesFontItalic));
	
	str = OLE2A(_options.valuesFontName);
	if (str != OLE2A(opt.valuesFontName))
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFontName", str);

	if (_options.valuesFontSize != opt.valuesFontSize)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFontSize", CPLString().Printf("%d", _options.valuesFontSize));

	if (_options.valuesFrameColor != opt.valuesFrameColor)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFrameColor", CPLString().Printf("%d", _options.valuesFrameColor));

	if (_options.valuesFrameVisible != opt.valuesFrameVisible)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesFrameVisible", CPLString().Printf("%d", (int)_options.valuesFrameVisible));
	
	if (_options.valuesStyle != opt.valuesStyle)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesStyle", CPLString().Printf("%d", (int)_options.valuesStyle));
	
	if (_options.valuesVisible != opt.valuesVisible)
		CPLCreateXMLAttributeAndValue(psTree, "ValuesVisible", CPLString().Printf("%d", (int)_options.valuesVisible));
	
	if (_options.verticalPosition != opt.verticalPosition)
		CPLCreateXMLAttributeAndValue(psTree, "VerticalPosition", CPLString().Printf("%d", (int)_options.verticalPosition));
	
	if (_options.visible != opt.visible)
		CPLCreateXMLAttributeAndValue(psTree, "Visible", CPLString().Printf("%d", (int)_options.visible));
	...
	*retVal = VARIANT_FALSE;
	...
	if (psTree)
	{
		...
		if (node)
		{
			long numShapes;
			_shapefile->get_NumShapes(&numShapes);
			CPLCreateXMLAttributeAndValue(node, "Count", CPLString().Printf("%d", numShapes));
