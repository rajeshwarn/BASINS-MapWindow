...
CPLXMLNode* CLabelCategory::SerializeCore(CString ElementName)
{
	...
	if (node)
	{
		...
		CPLCreateXMLAttributeAndValue( node, "Name", str);
		
		str = OLE2CA(m_expression);
		CPLCreateXMLAttributeAndValue( node, "Expression", str);
		CPLCreateXMLAttributeAndValue( node, "Enabled", CPLString().Printf("%d", (int)m_enabled ));
