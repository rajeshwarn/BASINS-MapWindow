...
STDMETHODIMP CLabels::put_Category(long Index, ILabelCategory* newVal)
{
	...
	else
	{
		...
		{
			put_ComReference(newVal, (IDispatch**)&m_categories[Index], false);
		}
 ...
 CPLXMLNode* CLabels::SerializeCore(CString ElementName)
{
	...
	if (m_labels.size() > 0)  		// positions should be saved only if there are labels
	{
		if (m_savingMode == modeStandard)
		{
			...
			CPLCreateXMLAttributeAndValue( psTree, "RotationSaved", CPLString().Printf("%d", (int)saveRotation));
			CPLCreateXMLAttributeAndValue( psTree, "TextSaved", CPLString().Printf("%d", (int)(saveText)));
	...
	CPLCreateXMLAttributeAndValue( psTree, "Generated", CPLString().Printf("%d", m_labels.size() > 0 ));
	
	// start labels specific options
	if (m_minDrawingSize != 1)
		CPLCreateXMLAttributeAndValue( psTree, "MinDrawingSize", CPLString().Printf("%d", m_minDrawingSize ));

	str = OLE2CA(m_expression);
	if (str != "")
		CPLCreateXMLAttributeAndValue( psTree, "VisibilityExpression", str);
	
	/*if (m_classificationField != -1)
		CPLCreateXMLAttributeAndValue( psTree, "ClassificationField", CPLString().Printf("%d", m_classificationField));*/
	...
	if (m_verticalPosition != vpAboveAllLayers)
		CPLCreateXMLAttributeAndValue( psTree, "VerticalPosition", CPLString().Printf("%d", (int)m_verticalPosition));

	if (m_basicScale != 0.0)
		CPLCreateXMLAttributeAndValue( psTree, "BasicScale", CPLString().Printf("%f", m_basicScale));

	if (m_maxVisibleScale != 100000000.0)
		CPLCreateXMLAttributeAndValue( psTree, "MaxVisibleScale", CPLString().Printf("%f", m_maxVisibleScale));
	
	if (m_minVisibleScale != 0.0)
		CPLCreateXMLAttributeAndValue( psTree, "MinVisibleScale", CPLString().Printf("%f", m_minVisibleScale));

	if (m_collisionBuffer != 0)
		CPLCreateXMLAttributeAndValue( psTree, "CollisionBuffer", CPLString().Printf("%d", m_collisionBuffer));

	if (m_scale != false)
		CPLCreateXMLAttributeAndValue( psTree, "ScaleLabels", CPLString().Printf("%d", (int)m_scale));

	if (m_autoOffset != VARIANT_TRUE)
		CPLCreateXMLAttributeAndValue( psTree, "AutoOffset", CPLString().Printf("%d", (int)m_autoOffset));

	if (m_dynamicVisibility != VARIANT_FALSE)
		CPLCreateXMLAttributeAndValue( psTree, "DynamicVisibility", CPLString().Printf("%d", (int)m_dynamicVisibility));

	if (m_avoidCollisions != VARIANT_TRUE)
		CPLCreateXMLAttributeAndValue( psTree, "AvoidCollisions", CPLString().Printf("%d", (int)m_avoidCollisions));

	if (m_useWidthLimits != VARIANT_FALSE)
		CPLCreateXMLAttributeAndValue( psTree, "UseWidthLimits", CPLString().Printf("%d", (int)m_useWidthLimits));

	if (m_removeDuplicates != VARIANT_FALSE)
		CPLCreateXMLAttributeAndValue( psTree, "RemoveDuplicates", CPLString().Printf("%d", (int)m_removeDuplicates));

	if (m_gdiPlus != VARIANT_TRUE)
		CPLCreateXMLAttributeAndValue( psTree, "UseGdiPlus", CPLString().Printf("%d", (int)m_gdiPlus));

	if (m_savingMode != modeXML)
		CPLCreateXMLAttributeAndValue( psTree, "SavingMode", CPLString().Printf("%d", (int)m_savingMode));

	if (m_positioning != lpNone)
		CPLCreateXMLAttributeAndValue( psTree, "Positioning", CPLString().Printf("%d", (int)m_positioning));

	if (m_textRenderingHint != HintAntiAlias)
		CPLCreateXMLAttributeAndValue( psTree, "TextRenderingHint", CPLString().Printf("%d", (int)m_textRenderingHint));

	if (m_smoothingMode != AntiAlias)
		CPLCreateXMLAttributeAndValue( psTree, "SmoothingMode", CPLString().Printf("%d", (int)m_smoothingMode));

	if (m_compositingQuality != HighQuality)
		CPLCreateXMLAttributeAndValue( psTree, "CompositingQuality", CPLString().Printf("%d", (int)m_compositingQuality));

	if (m_labelExpression != "")
		CPLCreateXMLAttributeAndValue( psTree, "Expression", m_labelExpression);
...
bool CLabels::DeserializeCore(CPLXMLNode* node)
{
	...
	s = CPLGetXMLValue( node, "SmoothingMode", NULL );
	m_smoothingMode = (s != "") ? (tkSmoothingMode)atoi(s.GetString()) : AntiAlias;

	s = CPLGetXMLValue( node, "CompositingQuality", NULL );
	m_compositingQuality = (s != "") ? (tkCompositingQuality)atoi(s.GetString()) : HighQuality;
...
STDMETHODIMP CLabels::SaveToXML(BSTR filename, VARIANT_BOOL* retVal)
{
	...
	*retVal = VARIANT_FALSE;
	...
	if (psTree)
	{
		...
		CPLCreateXMLAttributeAndValue( psTree, "FileVersion", CPLString().Printf("%d", 1));
		...
		if (node)
		{
			CPLCreateXMLAttributeAndValue( node, "Count", CPLString().Printf("%d", (int)m_labels.size()));
			CPLCreateXMLAttributeAndValue( node, "RotationSaved", CPLString().Printf("%d", (int)saveRotation));
			CPLCreateXMLAttributeAndValue( node, "TextSaved", CPLString().Printf("%d", (int)(saveText)));
...
CPLXMLNode* CLabels::SerializeLabelData(CString ElementName, bool& saveRotation, bool& saveText)
{
	...
	if (psLabels)
	{
		...
		for (unsigned int i = 0; i < m_labels.size(); i++)
		{
			...
			CLabelInfo* info = (*m_labels[i])[0];
			CPLCreateXMLAttributeAndValue(nodeOld, "X", CPLString().Printf("%f", info->x));
			CPLCreateXMLAttributeAndValue(nodeOld, "Y", CPLString().Printf("%f", info->y));
			if (saveRotation)
			{
				CPLCreateXMLAttributeAndValue(nodeOld, "Rotation", CPLString().Printf("%f", info->rotation));
			}
			if (saveText)
			{
				CPLCreateXMLAttributeAndValue(nodeOld, "Name", info->text);
			}
		}
	}
	return psLabels;
...
STDMETHODIMP CLabels::get_CompositingQuality(tkCompositingQuality* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	*pVal = m_compositingQuality;
	return S_OK;
}

STDMETHODIMP CLabels::put_CompositingQuality(tkCompositingQuality newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (newVal >= 0 && newVal <= AssumeLinear)
		m_compositingQuality = newVal;
	return S_OK;
}

STDMETHODIMP CLabels::get_SmoothingMode(tkSmoothingMode* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	*pVal = m_smoothingMode;
	return S_OK;
}

STDMETHODIMP CLabels::put_SmoothingMode(tkSmoothingMode newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (newVal >= 0 && newVal <= AntiAlias)
	m_smoothingMode = newVal;
	return S_OK;
}
#pragma endregion