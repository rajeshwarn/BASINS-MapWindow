...
STDMETHODIMP CLinePattern::put_Line(int Index, ILineSegment* newVal)
{
	...
	else
	{
		if (newVal)
		{
			put_ComReference(newVal, (IDispatch**)&_lines[Index], false);
		}
		else
		{
...
CPLXMLNode* CLinePattern::SerializeCore(CString ElementName)
{
	...
	CPLCreateXMLAttributeAndValue(psTree, "Key", CPLString().Printf(str));
	CPLCreateXMLAttributeAndValue(psTree, "Transparency", CPLString().Printf("%d", (int)_transparency));
	...
	if (_lines.size() > 0)
	{
		...
		if (psFields)
		{
			for (unsigned int i = 0; i < _lines.size(); i++)
			{
				...
				CPLCreateXMLAttributeAndValue(psNode, "LineType", CPLString().Printf("%d", (int)lineType));
				...
				CPLCreateXMLAttributeAndValue(psNode, "MarkerOutlineColor", CPLString().Printf("%d", color));

				// line width
				float val;
				_lines[i]->get_LineWidth(&val);
				CPLCreateXMLAttributeAndValue(psNode, "LineWidth", CPLString().Printf("%f", val));

				// marker size
				_lines[i]->get_MarkerSize(&val);
				CPLCreateXMLAttributeAndValue(psNode, "MarkerSize", CPLString().Printf("%f", val));

				// marker interval
				_lines[i]->get_MarkerInterval(&val);
				CPLCreateXMLAttributeAndValue(psNode, "MarkerInterval", CPLString().Printf("%f", val));

				// marker offset
				_lines[i]->get_MarkerOffset(&val);
				CPLCreateXMLAttributeAndValue(psNode, "MarkerOffset", CPLString().Printf("%f", val));

				// line style
				tkDashStyle lineStyle;
				_lines[i]->get_LineStyle(&lineStyle);
				CPLCreateXMLAttributeAndValue(psNode, "LineStyle", CPLString().Printf("%d", (int)lineStyle));

				// marker
				tkDefaultPointSymbol marker;
				_lines[i]->get_Marker(&marker);
				CPLCreateXMLAttributeAndValue(psNode, "Marker", CPLString().Printf("%d", (int)marker));

				// marker orientation
				tkLineLabelOrientation markerOrientation;
				_lines[i]->get_MarkerOrientation(&markerOrientation);
				CPLCreateXMLAttributeAndValue(psNode, "MarkerOrientation", CPLString().Printf("%d", (int)markerOrientation));

				// marker flip first
				VARIANT_BOOL markerFlipFirst;
				_lines[i]->get_MarkerFlipFirst(&markerFlipFirst);
				CPLCreateXMLAttributeAndValue(psNode, "MarkerFlipFirst", CPLString().Printf("%d", (int)markerFlipFirst));
			}
		}
	}
	return psTree;
