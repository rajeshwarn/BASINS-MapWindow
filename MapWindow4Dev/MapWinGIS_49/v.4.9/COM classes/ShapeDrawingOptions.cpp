...
STDMETHODIMP CShapeDrawingOptions::put_Picture(IImage* newVal)
{
	...
	else
	{
		...
		if (width > 0 && height > 0)
		{
			put_ComReference(newVal, (IDispatch**)&m_options.picture);
		}
	}
	return S_OK;
};
												 ...
												 int clipWidth, int clipHeight, OLE_COLOR backColor)
{
	...
	else
	{
		...
		//{
		//	rect.Width = m_options.pointSize * (float)pow(2.0, 0.5);
		//	rect.Height = m_options.pointSize * (float)pow(2.0, 0.5);
		//	rect.X -= rect.Width/2;
		//	rect.Y -= rect.Height/2;
		//	
		//	//rect.X -= m_options.pointSize/2;
		//	//rect.Y -= m_options.pointSize/2;
		//	
		//}
...
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	put_ComReference((IDispatch*)newVal, (IDispatch**)&(m_options.linePattern), false);
	return S_OK;
}
...
CPLXMLNode* CShapeDrawingOptions::SerializeCore(CString ElementName)
{
	...
	if (opt->fillBgColor != m_options.fillBgColor)
		CPLCreateXMLAttributeAndValue(psTree, "FillBgColor", CPLString().Printf("%d", m_options.fillBgColor));

	if (opt->fillBgTransparent != m_options.fillBgTransparent)
		CPLCreateXMLAttributeAndValue(psTree, "FillBgTransparent", CPLString().Printf("%d", (int)m_options.fillBgTransparent));

	if (opt->fillColor != m_options.fillColor)
		CPLCreateXMLAttributeAndValue(psTree, "FillColor", CPLString().Printf("%d", m_options.fillColor));

	if (opt->fillColor2 != m_options.fillColor2)
		CPLCreateXMLAttributeAndValue(psTree, "FillColor2", CPLString().Printf("%d", m_options.fillColor2));
	
	if (opt->fillGradientBounds != m_options.fillGradientBounds)
		CPLCreateXMLAttributeAndValue(psTree, "FillGradientBounds", CPLString().Printf("%d", (int)m_options.fillGradientBounds));
	
	if (opt->fillGradientRotation != m_options.fillGradientRotation)
		CPLCreateXMLAttributeAndValue(psTree, "FillGradientRotation", CPLString().Printf("%f", m_options.fillGradientRotation));
	
	if (opt->fillGradientType != m_options.fillGradientType)
		CPLCreateXMLAttributeAndValue(psTree, "FillGradientType", CPLString().Printf("%d", (int)m_options.fillGradientType));
	
	if (opt->fillHatchStyle != m_options.fillHatchStyle)
		CPLCreateXMLAttributeAndValue(psTree, "FillHatchStyle", CPLString().Printf("%d", (int)m_options.fillHatchStyle));
	
	if (opt->fillTransparency != m_options.fillTransparency)
		CPLCreateXMLAttributeAndValue(psTree, "FillTransparency", CPLString().Printf("%f", m_options.fillTransparency));
	
	if (opt->fillType != m_options.fillType)
		CPLCreateXMLAttributeAndValue(psTree, "FillType", CPLString().Printf("%d", (int)m_options.fillType));
	
	if (opt->fillVisible != m_options.fillVisible)
		CPLCreateXMLAttributeAndValue(psTree, "FillVisible", CPLString().Printf("%d", (int)m_options.fillVisible));
	
	if (opt->fontName != m_options.fontName)
		CPLCreateXMLAttributeAndValue(psTree, "FontName", m_options.fontName);
	...
	if (opt->linesVisible != m_options.linesVisible)
		CPLCreateXMLAttributeAndValue(psTree, "LinesVisible", CPLString().Printf("%d", (int)m_options.linesVisible));
	
	if (opt->lineWidth != m_options.lineWidth)
		CPLCreateXMLAttributeAndValue(psTree, "LineWidth", CPLString().Printf("%f", m_options.lineWidth));
	
	if (opt->pointCharcter != m_options.pointCharcter)
		CPLCreateXMLAttributeAndValue(psTree, "PointCharcter", CPLString().Printf("%d", (int)m_options.pointCharcter));
	
	if (opt->pointColor != m_options.pointColor)
		CPLCreateXMLAttributeAndValue(psTree, "PointColor", CPLString().Printf("%d", m_options.pointColor));
	
	if (opt->pointNumSides != m_options.pointNumSides)
		CPLCreateXMLAttributeAndValue(psTree, "PointNumSides", CPLString().Printf("%d", m_options.pointNumSides));
	
	if (opt->pointShapeRatio != m_options.pointShapeRatio)
		CPLCreateXMLAttributeAndValue(psTree, "PointShapeRatio", CPLString().Printf("%f", m_options.pointShapeRatio));
	
	if (opt->pointShapeType != m_options.pointShapeType)
		CPLCreateXMLAttributeAndValue(psTree, "PointShapeType", CPLString().Printf("%d", (int)m_options.pointShapeType));
	
	if (opt->pointSize != m_options.pointSize)
		CPLCreateXMLAttributeAndValue(psTree, "PointSize", CPLString().Printf("%f", m_options.pointSize));
	
	if (opt->pointSymbolType != m_options.pointSymbolType)
		CPLCreateXMLAttributeAndValue(psTree, "PointSymbolType", CPLString().Printf("%d", (int)m_options.pointSymbolType));
	
	if (opt->rotation != m_options.rotation)
		CPLCreateXMLAttributeAndValue(psTree, "Rotation", CPLString().Printf("%f", m_options.rotation));
	
	if (opt->scaleX != m_options.scaleX)
		CPLCreateXMLAttributeAndValue(psTree, "ScaleX", CPLString().Printf("%f", m_options.scaleX));
	
	if (opt->scaleY != m_options.scaleY)
		CPLCreateXMLAttributeAndValue(psTree, "ScaleY", CPLString().Printf("%f", m_options.scaleY));
	
	if (opt->verticesColor != m_options.verticesColor)
		CPLCreateXMLAttributeAndValue(psTree, "VerticesColor", CPLString().Printf("%d", m_options.verticesColor));
	
	if (opt->verticesFillVisible != m_options.verticesFillVisible)
		CPLCreateXMLAttributeAndValue(psTree, "VerticesFillVisible", CPLString().Printf("%d", (int)m_options.verticesFillVisible));
	
	if (opt->verticesSize != m_options.verticesSize)
		CPLCreateXMLAttributeAndValue(psTree, "VerticesSize", CPLString().Printf("%d", m_options.verticesSize));
	
	if (opt->verticesType != m_options.verticesType)
		CPLCreateXMLAttributeAndValue(psTree, "VerticesType", CPLString().Printf("%d", (int)m_options.verticesType));
	
	if (opt->verticesVisible != m_options.verticesVisible)
		CPLCreateXMLAttributeAndValue(psTree, "VerticesVisible", CPLString().Printf("%d", (int)m_options.verticesVisible));
		
	if (opt->visible != m_options.visible)
		CPLCreateXMLAttributeAndValue(psTree, "Visible", CPLString().Printf("%d", (int)m_options.visible));

	if (opt->useLinePattern != m_options.useLinePattern)
		CPLCreateXMLAttributeAndValue(psTree, "UseLinePattern", CPLString().Printf("%d", (int)m_options.useLinePattern));
