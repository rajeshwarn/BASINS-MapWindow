...
CPLXMLNode* CShapefileCategories::SerializeCore(CString ElementName)
{
	...
	if (serializeField)
	{
		...
		if (table)
		{
			...
			if (fld)
			{
				...
				CPLCreateXMLAttributeAndValue( psTree, "FieldType", CPLString().Printf("%d", (int)type));
				serializeField = true;
	...
	for (unsigned int i = 0; i < m_categories.size(); i++)
	{
		...
		CPLCreateXMLAttributeAndValue( psNode, "Name", OLE2CA(vbstr));
		SysFreeString(vbstr);
		...
		CPLCreateXMLAttributeAndValue( psNode, "Expression", str);			//CPLString().Printf(str)
		SysFreeString(vbstr);
		...
		if (serializeField)
		{
			...
			{
				CPLCreateXMLAttributeAndValue( psNode, "Value", OLE2CA(val->bstrVal));
			}
			else if (type == DOUBLE_FIELD)
			{
				CPLCreateXMLAttributeAndValue( psNode, "Value", CPLString().Printf("%f",val->dblVal));
			}
			else if (type == INTEGER_FIELD)
			{
				CPLCreateXMLAttributeAndValue( psNode, "Value", CPLString().Printf("%d", val->lVal));
			}
		}
