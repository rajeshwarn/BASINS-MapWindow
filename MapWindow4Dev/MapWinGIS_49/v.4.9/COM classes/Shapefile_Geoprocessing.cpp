...
//	in definite spatial relation to the shapes of the input shapefile. 
	...
	*retval = VARIANT_FALSE;
	...
	if (Relation == srDisjoint)		
	{
		...
		for (int i = 0; i < _numShapes1; i++)
		{
			...
			{
				if (*p == i)
					include = false;	// this shpae actually intersects, so it can't be disjointed
...
STDMETHODIMP CShapefile::PointInShape(LONG ShapeIndex, DOUBLE x, DOUBLE y, VARIANT_BOOL* retval)
{
	...
	{	
		...
		IShape * shp;
		IPoint * pnt;
		//shp = memShapes[ShapeIndex];
		shp = _shapeData[ShapeIndex]->shape;
