...
STDMETHODIMP CShapefile::QuickPoints(long ShapeIndex, long *NumPoints, SAFEARRAY ** retval)
{
	...
	*NumPoints = 0;
	...
	else
	{
		if( _isEditingShapes == TRUE )
		{	
			//memShapes[ShapeIndex]->get_NumPoints(NumPoints);
			...
			if( *NumPoints > 0 )
			{
				*retval = SafeArrayCreateVector(VT_R8,0,(*NumPoints)*2);
				...
				{
					_shapeData[ShapeIndex]->shape->get_Point(i, &pnt);
					//memShapes[ShapeIndex]->get_Point(i,&pnt);					
					if( pnt )
					{	
						pnt->get_X(&x);
						pnt->get_Y(&y);	
...
STDMETHODIMP CShapefile::QuickExtents(long ShapeIndex, IExtents **retval)
{
	...
	*retval = NULL;
	...
	{			
		if( _isEditingShapes == TRUE)
		{		
			_shapeData[ShapeIndex]->shape->get_Extents(retval);
		}
		else
		{				
			if (_fastMode)
			{
				...
				(*retval)->SetBounds(xMin, yMin, 0, xMax, yMax, 0);
			}
			else
			{
				//Get the Info from the disk
				...
				else
				{
					...
					{	
						...
						(*retval)->SetBounds(bnds[0],bnds[1],0,bnds[0],bnds[1],0);					
					}
					else if( shapetype == SHP_POLYLINE || shapetype == SHP_POLYLINEZ || shapetype == SHP_POLYLINEM ||
					...
					{	
						...
						(*retval)->SetBounds(bnds[0],bnds[1],0,bnds[2],bnds[3],0);					
					}				
					else
					{	
						ErrorMessage( tkUNSUPPORTED_SHAPEFILE_TYPE );
