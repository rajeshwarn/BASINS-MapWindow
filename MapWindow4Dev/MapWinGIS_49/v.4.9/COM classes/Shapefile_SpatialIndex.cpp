...
{
	//08-24-2009 (sm) spatial index tweaks
	//Check that the spatial index exists, is set to be used and should be used.
	*pVal = VARIANT_FALSE;

	if (_isEditingShapes == FALSE && useSpatialIndex)
	{
		...
		if (spatialIndexExists)
		{
			...
			if (selectShapeArea/shapeFileArea < spatialIndexMaxAreaPercent) //when large portions of the map are being drawn,
																			//the spatial index *probably* won't help, don't use it.
			{
				...
				else
				{
					...
					if (loadSpatialIndex(baseName, false, spatialIndexNodeCapacity, spatialIndexID))
					{
...
STDMETHODIMP CShapefile::CreateSpatialIndex(BSTR ShapefileName, VARIANT_BOOL *retval)
{
	...
	*retval = VARIANT_TRUE;
	...
	else
	{
		...
		try 
		{
			if (!createSpatialIndex(0.9, spatialIndexNodeCapacity, (char *)baseName.c_str()))
			{	
				*retval = VARIANT_FALSE;
				ErrorMessage(tkINVALID_FILENAME);
			}
...
STDMETHODIMP CShapefile::IsSpatialIndexValid(VARIANT_BOOL *retval)
{
	...
	{
		string baseName = _shpfileName.Left(_shpfileName.GetLength() - 4);
		bool bIsValid = isValidSpatialIndex(baseName.c_str(), spatialIndexNodeCapacity);
