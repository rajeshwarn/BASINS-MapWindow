...
BEGIN_DISPATCH_MAP(CMapView, COleControl)
	...
	DISP_PROPERTY_EX_ID(CMapView, "GeographicalExtents", dispidGeographicalExtents, GetGeographicalExtents, SetGeographicalExtents, VT_DISPATCH)
...
void CMapView::OnSize(UINT nType, int cx, int cy)
{
	...
	// resizing layers buffer
	//if ((HGDIOBJ)m_layersBitmap)
	//	VERIFY(m_layersBitmap.DeleteObject());  
	
	//VERIFY(m_layersBitmap.CreateDiscardableBitmap(pDC,cx,cy));
	//m_layerDC->SelectObject(&m_layersBitmap);
	
	// resizing tiles buffer
	/*if ((HGDIOBJ)m_tilesBitmap)
		VERIFY(m_tilesBitmap.DeleteObject()); 

	VERIFY(m_tilesBitmap.CreateDiscardableBitmap(pDC,cx,cy));
	m_tilesDC->SelectObject(&m_tilesBitmap);*/

	// resizing main buffer
	/*if ((HGDIOBJ)m_bufferBitmap)
		VERIFY(m_bufferBitmap.DeleteObject()); 

	VERIFY(m_bufferBitmap.CreateDiscardableBitmap(pDC,cx,cy));
...
void CMapView::OnDraw(CDC* pdc, const CRect& rcBounds, const CRect& rcInvalid)
{
	...
	#ifdef _DEBUG
	CLSID pngClsid;
	Utility::GetEncoderClsid(L"image/png", &pngClsid);	// perhaps some other formats ?
	#endif

	// -------------------------------------------------
	// let's determine what we need to redraw:
	// -------------------------------------------------
	//bool tilesRedraw = ((CTiles*)m_tiles)->m_tilesLoaded;
	bool layersRedraw = !m_canbitblt && m_lockCount == 0;
	...
	if (dragging)
	{
		...
		//pdc->BitBlt( m_bitbltClickMove.x - m_bitbltClickDown.x, m_bitbltClickMove.y - m_bitbltClickDown.y, 
		//			 rcBounds.Width(), rcBounds.Height(), m_bufferDC, 0, 0, SRCCOPY);

		int x = m_bitbltClickMove.x - m_bitbltClickDown.x;
		int y = m_bitbltClickMove.y - m_bitbltClickDown.y;

		Gdiplus::Graphics* gBuffer = Gdiplus::Graphics::FromHDC(pdc->GetSafeHdc());
		gBuffer->DrawImage(m_bufferBitmap, (Gdiplus::REAL)x, (Gdiplus::REAL)y);
		gBuffer->ReleaseHDC(pdc->GetSafeHdc());
		delete gBuffer;

		// drawing layers
		Gdiplus::Graphics* g = Gdiplus::Graphics::FromHDC(pdc->GetSafeHdc());
	...
	else
	{
		...
		if (tilesVisible && m_transformationMode != tmNotDefined)	// projection isn't defined so there is no way to display tiles
		{
			...
			if (tiles->UndrawnTilesExist())
			{
				...
				drawer.DrawTiles(this->m_tiles, this->PixelsPerMapUnit());
			}
...
void CMapView::DrawLayers(const CRect & rcBounds, Gdiplus::Graphics* graphics)
{
	...
	{
		// drawing
		//this->DrawLists(rcBounds,dc,dlSpatiallyReferencedList);		// TODO: move to the OnDraw
		m_drawMutex.Unlock();
		::SetCursor(oldCursor);
		return;
	}
	...
	bool forceGdiplus = this->m_RotateAngle != 0.0f;
	CShapefileDrawer sfDrawer(graphics, &extents, m_pixelPerProjectionX, m_pixelPerProjectionY, &m_collisionList, forceGdiplus);
	CImageDrawer imgDrawer(graphics, &extents, m_pixelPerProjectionX, m_pixelPerProjectionY, m_viewWidth, m_viewHeight);
	CLabelDrawer lblDrawer(graphics, &extents, m_pixelPerProjectionX, m_pixelPerProjectionY, currentScale, chosenListLabels, m_RotateAngle);
	...
	// drawing
	this->DrawLists(rcBounds, graphics, dlSpatiallyReferencedList);
...
DOUBLE CMapView::GetCurrentScale(void)
{
	...
	{
		//TODO: reimplement
		long pixX = 96.0; //m_layerDC->GetDeviceCaps(LOGPIXELSX);	// Number of pixels per logical inch along the screen width.
		long pixY = 96.0; //m_layerDC->GetDeviceCaps(LOGPIXELSY);	// Number of pixels per logical inch along the screen height
...
LPDISPATCH CMapView::SnapShot(LPDISPATCH BoundBox)
{
		...
		top == extents.top )
	{
		...
		{
			this->DrawOldLabels(cdc);
		}
		//this->DrawLists(rcBounds,cdc,dlScreenReferencedList);
...
DOUBLE CMapView::GetPixelsPerDegree(void)
{
	...
	{
		VARIANT_BOOL vbretval;
		m_projection->get_IsEmpty(&vbretval);

		OGRSpatialReference* ref = ((CGeoProjection*)m_projection)->get_SpatialReference();
		double val = ref->GetAngularUnits();
		if (val != 1.0)
		{
...
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	put_ComReference(pVal, (IDispatch**)&m_projection, false);
	
	if (m_transformationMode == tmDoTransformation)
		m_wgsProjection->StopTransform();
	
	VARIANT_BOOL isSame;
	if (!m_wgsProjection->get_IsSame(m_projection, &isSame));
	if (isSame)
	{
	...
	else
	{
		...
		if (vbretval)
		{
			/*double xMin = 27.0;
			double yMin = 54.0;
			m_wgsProjection->Transform(&xMin, &yMin, &vbretval);
			if (vbretval)
			{
				double xMax = 180;
				double yMax = 90.0;
				m_wgsProjection->Transform(&xMax, &yMax, &vbretval);
				if (vbretval)
				{
					m_maxExtents = new Extent(xMin, xMax, yMin, yMax);
				}
			}*/
		}
	}
...
//		GetProjection()
// *****************************************************
IExtents* CMapView::GetGeographicalExtents(void)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	if (m_projection)
	{
		...
		else
		{
			...
			if (vbretval)
			{
				...
				IExtents * box = NULL;
				CoCreateInstance( CLSID_Extents, NULL, CLSCTX_INPROC_SERVER, IID_IExtents, (void**)&box);
				box->SetBounds( xTL, yBR, 0, xBR, yTL, 0 );		// TODO: return 4 point geographical extents
																// as projections other that equirectangular can be used
				return box;
cleaning:
...
//		SetProjection()
// *****************************************************
void CMapView::SetGeographicalExtents(IExtents* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (!pVal)
	{
	...
	else
	{
		...
		else if (m_transformationMode == tmDoTransformation)
		{
			...
			VARIANT_BOOL vbretval;
			m_wgsProjection->Transform(&xMin, &yMin, &vbretval);	 if (!vbretval) return;
			m_wgsProjection->Transform(&xMax, &yMax, &vbretval);	 if (!vbretval) return;
