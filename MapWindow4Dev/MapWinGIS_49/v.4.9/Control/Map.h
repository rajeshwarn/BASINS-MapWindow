...
public:
	enum {		//{{AFX_DISP_ID(CMapView)
		dispidProjection = 193,
		dispidGeographicalExtents = 192,
		dispidTiles = 191L,
...
#pragma region Drawing
	...
	bool m_forceBounds;		// tws added 4/7/2007: used by SnapShot2 to coerce DrawImage() to use the desired size rather than the screen size
...
protected:
	...
	VARIANT_BOOL CMapView::get_Fileame(LONG LayerHandle, BSTR* layerName );
	...
	ITiles* GetTiles(void);
	void SetTiles(ITiles* pVal);
...
protected:
	IExtents* GetGeographicalExtents(void);
	void SetGeographicalExtents(IExtents* pVal);
	IGeoProjection* GetGeoProjection(void);
	void SetGeoProjection(IGeoProjection* pVal);
