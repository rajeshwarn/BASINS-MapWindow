...
long CMapView::GetDrawingLabelsOffset(long drawHandle)
{
	if( IsValidDrawList(drawHandle) )
	{
		...
		{
			return m_allDrawLists[drawHandle]->labelsOffset;
		}
		else
		{
...
void CMapView::SetDrawingLabelsOffset(long drawHandle, long Offset)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->labelsOffset = Offset;
			m_canbitblt = FALSE;
			if( !m_lockCount )
				InvalidateControl();
		}
...
OLE_COLOR CMapView::GetDrawingLabelsShadowColor(long drawHandle)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			return m_allDrawLists[drawHandle]->shadowColor;
		}
		else
		{
			OLE_COLOR clr;
...
void CMapView::SetDrawingLabelsShadowColor(long drawHandle, OLE_COLOR color)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->shadowColor = color;
		}
		else
		{
...
void CMapView::SetDrawingLabelsShadow(long drawHandle, BOOL newValue)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->shadowLabels = newValue;
		}
		else
		{
...
BOOL CMapView::GetDrawingLabelsShadow(long drawHandle)
{
	...
	{	
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			return m_allDrawLists[drawHandle]->shadowLabels;
		}
		else
		{
...
void CMapView::SetDrawingLabelsScale(long drawHandle, BOOL newValue)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->scaleLabels = newValue;
		}
		else
		{
...
BOOL CMapView::GetDrawingLabelsScale(long drawHandle)
{
	...
	{	
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			return m_allDrawLists[drawHandle]->scaleLabels;
		}
		else
		{
			VARIANT_BOOL scale;
...
BOOL CMapView::GetUseDrawingLabelCollision(long drawHandle)
{
	...
	{	
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			return m_allDrawLists[drawHandle]->useLabelCollision;
		}
		else
		{
			VARIANT_BOOL val;
...
void CMapView::SetUseDrawingLabelCollision(long drawHandle, BOOL value)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->useLabelCollision = value;
			if(value == TRUE)
			{
...
BOOL CMapView::GetDrawingLabelsVisible(long drawHandle)
{
	...
	{	
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			return m_allDrawLists[drawHandle]->visibleLabels;
		}
		else
		{
			VARIANT_BOOL visible;
...
void CMapView::SetDrawingLabelsVisible(long drawHandle, BOOL bNewValue)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->visibleLabels = bNewValue;
		}
		else
		{
...
void CMapView::GetDrawingStandardViewWidth(long drawHandle, double * Width)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			*Width = m_allDrawLists[drawHandle]->standardViewWidth;
		}
		else
		{
			*Width = 0.0;
...
void CMapView::SetDrawingStandardViewWidth(long drawHandle, double Width)
{
	if( IsValidDrawList(drawHandle) )
	{
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			m_allDrawLists[drawHandle]->standardViewWidth = Width;
			m_canbitblt = FALSE;
			if( !m_lockCount )
				InvalidateControl();
		}
...
{
	return;
	// TODO: implement proper collision list fr drawing labels; frames for drawing labels should be cleared from list while frames from the rest layers shold remain 
	// in case Map.Refresh() was called
	CCollisionList collisionList;

	// label drawer for spatially referenced list
	CLabelDrawer lblDrawer(graphics, &extents, m_pixelPerProjectionX, m_pixelPerProjectionY, this->GetCurrentScale(), &collisionList, m_RotateAngle );
	...
	for(unsigned int j = 0; j < m_activeDrawLists.size(); j++ )
	{
		...
		if( IS_VALID_PTR(dlist) )
		{
			if (dlist->listType == listType)
			{
				...
				if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
				{
					m_labelsDrawingLists.push_back(dlist);
				}
				else
				{
...
void CMapView::ClearDrawing(long Drawing)
{
	...
	if( IsValidDrawList (Drawing) )
	{
		...
		if( m_allDrawLists[Drawing] )
		{
			...
			endcondition = m_labelsDrawingLists.size();
			for(int i = 0; i < endcondition; i++ ) 
			{
				if( m_labelsDrawingLists[i] == m_allDrawLists[Drawing] ) 
				{
					m_labelsDrawingLists.erase( m_labelsDrawingLists.begin() + i );
					break;
				}
			}
...
void CMapView::AddDrawingLabel(long drawHandle, LPCTSTR Text, OLE_COLOR Color, double x, double y, short hJustification)
{
	...
	{	
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			...
			m_allDrawLists[drawHandle]->allLabels.push_back( l );
		}
		else
		{
...
void CMapView::ClearDrawingLabels(long drawHandle)
{
	if( IsValidDrawList(drawHandle) )
	{	
		m_allDrawLists[drawHandle]->allLabels.clear();
...
void CMapView::DrawingFont(long drawHandle, LPCTSTR FontName, long FontSize)
{
	if( IsValidDrawList(drawHandle) )
	{
		...
		if ((m_ShapeDrawingMethod == dmStandard || m_ShapeDrawingMethod == dmNewWithSelection) && !FORCE_NEW_LABELS)
		{
			dlist->fontSize = FontSize;
			dlist->fontName = FontName;
			...
			dlist->SetFont(new CFont);

			VERIFY(dlist->GetFont()->CreatePointFont(dlist->fontSize*10,dlist->fontName));
		}
		else
		{
...
long CMapView::NewDrawing(short Projection)
{
	...
	//Default Font
	DrawingFont( drawHandle, dlist->fontName, dlist->fontSize );
	return drawHandle;
}
...
void CMapView::SetDrawingLabels(long DrawingLayerIndex, ILabels* newVal) //LPDISPATCH newVal)
{
	...
	if( IsValidDrawList(DrawingLayerIndex))
	{	
		put_ComReference((IDispatch*)newVal, (IDispatch**)&m_allDrawLists[DrawingLayerIndex]->m_labels, false);
		
		/*ILabels * labels = NULL;
		newVal->QueryInterface(IID_ILabels, (void**)&labels);
		m_allDrawLists[DrawingLayerIndex]->m_labels->Release();
		m_allDrawLists[DrawingLayerIndex]->m_labels = NULL;
		m_allDrawLists[DrawingLayerIndex]->m_labels = labels;*/
	}
	else
		ErrorMessage(tkINVALID_DRAW_HANDLE);
}
