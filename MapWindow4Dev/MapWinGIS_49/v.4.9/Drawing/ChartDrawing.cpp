...
void CChartDrawer::DrawCharts(IShapefile* sf)
{
	...
	// creating brushes
	_dc->EndPath();
	_dc->SetGraphicsMode(GM_COMPATIBLE);

	long transp = _options->transparency ;
	long alpha = long(transp) << 24;
	Gdiplus::Graphics graphics(_dc->m_hDC);
	...
	if (_options->chartType == chtPieChart)
	{
		...
		Gdiplus::REAL offsetX = (Gdiplus::REAL)_offsetX;//- pieWidth/2.0f;
		Gdiplus::REAL offsetY = (Gdiplus::REAL)_offsetY;//- pieHeight/2.0f;
		...
		for (unsigned int k = 0; k < arr.size(); k++)
		{
			...
			// -----------------------------------------------------
			//   testing if we can to draw labels
			// -----------------------------------------------------
	...
	else // if ( _chartType == chtBarChart )
	{
		...
		for (unsigned int k = 0; k < arr.size(); k++)
		{
			...
			for (int j = 0; j < numBars; j++)
			{
				...
				{
					// drawing bars
					graphics.FillRectangle(brushes[j], xStart, yStart - height, _options->barWidth, height);
					graphics.DrawRectangle(&pen, xStart, yStart - height, _options->barWidth, height);
					...
					if ( _options->use3Dmode )
					{
						...
						graphics.FillPolygon(brushesDimmed[j], points, 5);
