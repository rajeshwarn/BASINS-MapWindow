...
void CDrawingOptionsEx::InitGdiPen(bool selection, OLE_COLOR selectionColor)
{
	...
	{
		pen->CreatePen(PS_NULL, (int)this->lineWidth, color);
	}
	else
	{
		if( this->lineStipple == lsNone )
		{
			pen->CreatePen(PS_SOLID, (int)this->lineWidth, color);
		}
		else
		{
			...
			if (this->lineWidth >1)
			{
				...
				pen->CreatePen(PS_GEOMETRIC|penStyle, (int)this->lineWidth, &logBrush, 0, 0);
			}
			else
			{
				pen->CreatePen(penStyle,(int)this->lineWidth,color);
			}
		}
	}
}
