...
ScreenBitmap* CImageDrawer::DrawImage(const CRect & rcBounds, IImage* iimg, bool returnBitmap)
{
	...
	HDC hdc = NULL;	// for GDI mode

	if (!useGDIplus)
	{
		hdc = _graphics->GetHDC();		// release the pointer after the usage
	...
	{
		//gr = new Gdiplus::Graphics(_dc->m_hDC);
			...
			if (WithinVisibleExtents(mapL,mapR, mapB,mapT))
			{
				...
				if (useGDIplus)
				{
					...
					if (destSize > (imgW * imgH))
						_graphics->SetInterpolationMode((Gdiplus::InterpolationMode)upsamplingMode);
					else
						_graphics->SetInterpolationMode((Gdiplus::InterpolationMode)downsamplingMode);
	...
	{	
		...
		for(int row = 0; row < imgHeight; )
		{
			...
			if (WithinVisibleExtents(mapL,mapR, mapB,mapT))
			{
				...
				_graphics->SetPixelOffsetMode(Gdiplus::PixelOffsetModeDefault);
				...
				if (useGDIplus)
				{
					...
					if (returnBitmap)
					{
						...
						_graphics->DrawImage(screenBitmap->bitmap, (Gdiplus::REAL)(dstL), (Gdiplus::REAL)(dstT));
