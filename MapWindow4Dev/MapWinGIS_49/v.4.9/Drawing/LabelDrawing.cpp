...
CLabelDrawer::CLabelDrawer(Gdiplus::Graphics* graphics, Extent* extents, double pixelPerProjectionX, double pixelPerProjectionY, double currentScale, 
						 CCollisionList* collisionList, double mapRotation)
{		
	_extents = extents;
...
void CLabelDrawer::DrawLabels( ILabels* LabelsClass )
{
	...
	// do we need to enable auto offset? shapfile type should be point and labels should be synchronized
	...
	if (synchronized && sf)
	{
		...
		{
			int index = arrInit[i];
			if ((*shapeData)[index]->size >= minSize)  // /*(*shapeData)[index]->isVisible && */
			{
				arr.push_back(index);
			}
		}
	...
	//long pos;
	//ILabelCategory* cat;
	//std::multimap<long, int> mmap;				// priority as key and index as value

	//for (int iCategory = 0; iCategory < numCategories; iCategory++)
	//{
	//	LabelsClass->get_Category(iCategory, &cat);
	//	if (cat == NULL) continue;
	//	cat->get_Priority(&pos);
	//	cat->Release();
	//	
	//	pair<long, int> myPair(pos, iCategory);	
	//	mmap.insert(myPair);
	//} 
	//
	//// for uncategorized labels
	//pair<long, int> myPair(0, -1);	
	//mmap.insert(myPair);
	...
	else
	{
		...
		Gdiplus::CompositingQuality compositingQualityInit = _graphics->GetCompositingQuality();
		Gdiplus::SmoothingMode smoothingModeInit = _graphics->GetSmoothingMode();
		Gdiplus::TextRenderingHint textRenderingHintInit = _graphics->GetTextRenderingHint();
		
		tkTextRenderingHint textRenderingHint;
		LabelsClass->get_TextRenderingHint(&textRenderingHint);

		tkSmoothingMode smoothingMode;
		LabelsClass->get_SmoothingMode(&smoothingMode);

		tkCompositingQuality compositingQuality;
		LabelsClass->get_CompositingQuality(&compositingQuality);
	...
	for (int iCategory = numCategories - 1; iCategory >= -1; iCategory--)
	{
		...
		for (unsigned int k = 0; k < size; k++ )
		{
			...
			for (int j =0; j < (int)parts->size(); j++ )
			{
				...
				//// we don't want our labels to be upside-down
				//double angle = long(lbl->rotation) % 360;
				//if (angle > 90.0 && angle < 180)			angle += 180.0;
				//else if (angle >= 180.0 && angle < 270.0)	angle -= 180.0;
				//double angleRad = angle/180.0 * pi;
				
				// --------------------------------------------------------------
				//	Checking for label collision pointers to label frames 
				//  are written to both LabelClass and m_labelExtents vectors
				// --------------------------------------------------------------
				//if (lbl->rotation != 0.0)
				...
				else
				{
					...
					if (m_options->fontTransparency != 0)
					{
						...
						//_graphics->SetTextRenderingHint(Gdiplus::TextRenderingHintSystemDefault);
