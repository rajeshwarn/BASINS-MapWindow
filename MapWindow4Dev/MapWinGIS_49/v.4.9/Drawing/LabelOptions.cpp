...
void CLabelOptions::DeserializeFromNode(CPLXMLNode* node)
{
	if (node)	// it's assumed here that a coorect node is passed
	{
		...
		*this = opt;
		...
		if (s != "") this->offsetX = (double)atoi(s.GetString());

		s = CPLGetXMLValue( node, "OffsetY", NULL );
		if (s != "") this->offsetY = (double)atoi(s.GetString());
...
CPLXMLNode* CLabelOptions::SerializeToTree(CString elementName )
{
	...
	if (opt->alignment != this->alignment)
		CPLCreateXMLAttributeAndValue( psTree, "Alignment", CPLString().Printf("%d", (int)this->alignment));

	if (opt->fontColor != this->fontColor)
		CPLCreateXMLAttributeAndValue( psTree, "FontColor", CPLString().Printf("%d", this->fontColor ));
	
	if (opt->fontColor2 != this->fontColor2)
		CPLCreateXMLAttributeAndValue( psTree, "FontColor2", CPLString().Printf("%d", this->fontColor2 ));
	
	if (opt->fontGradientMode != this->fontGradientMode)
		CPLCreateXMLAttributeAndValue( psTree, "FontGradientMode", CPLString().Printf("%d", this->fontGradientMode ));
	
	CString str = OLE2CA(this->fontName);
	CString str2 = OLE2CA(opt->fontName);
	if (str != str2)
		CPLCreateXMLAttributeAndValue( psTree, "FontName", str);
	
	if (opt->fontOutlineColor != this->fontOutlineColor)
		CPLCreateXMLAttributeAndValue( psTree, "FontOutlineColor", CPLString().Printf("%d", this->fontOutlineColor ));
	
	if (opt->fontOutlineVisible != this->fontOutlineVisible)
		CPLCreateXMLAttributeAndValue( psTree, "FontOutlineVisible", CPLString().Printf("%d", (int)this->fontOutlineVisible ));
	
	if (opt->fontOutlineWidth != this->fontOutlineWidth)
		CPLCreateXMLAttributeAndValue( psTree, "FontOutlineWidth", CPLString().Printf("%d", this->fontOutlineWidth ));
	
	if (opt->fontSize != this->fontSize)
		CPLCreateXMLAttributeAndValue( psTree, "FontSize", CPLString().Printf("%d", this->fontSize ));
	
	if (opt->fontStyle != this->fontStyle)
		CPLCreateXMLAttributeAndValue( psTree, "FontStyle", CPLString().Printf("%d", this->fontStyle ));
	
	if (opt->fontTransparency != this->fontTransparency)
		CPLCreateXMLAttributeAndValue( psTree, "FontTransparency", CPLString().Printf("%d", this->fontTransparency ));
	
	if (opt->frameBackColor != this->frameBackColor)
		CPLCreateXMLAttributeAndValue( psTree, "FrameBackColor", CPLString().Printf("%d", this->frameBackColor ));
	
	if (opt->frameBackColor2 != this->frameBackColor2)
		CPLCreateXMLAttributeAndValue( psTree, "FrameBackColor2", CPLString().Printf("%d", this->frameBackColor2 ));
	
	if (opt->frameGradientMode != this->frameGradientMode)
		CPLCreateXMLAttributeAndValue( psTree, "FrameGradientMode", CPLString().Printf("%d", (int)this->frameGradientMode ));
	
	if (opt->frameOutlineColor != this->frameOutlineColor)
		CPLCreateXMLAttributeAndValue( psTree, "FrameOutlineColor", CPLString().Printf("%d", this->frameOutlineColor ));
	
	if (opt->frameOutlineStyle != this->frameOutlineStyle)
		CPLCreateXMLAttributeAndValue( psTree, "FrameOutlineStyle", CPLString().Printf("%d", (int)this->frameOutlineStyle ));
	
	if (opt->frameOutlineWidth != this->frameOutlineWidth)
		CPLCreateXMLAttributeAndValue( psTree, "FrameOutlineWidth", CPLString().Printf("%d", this->frameOutlineWidth ));
	
	if (opt->framePaddingX != this->framePaddingX)
		CPLCreateXMLAttributeAndValue( psTree, "FramePaddingX", CPLString().Printf("%d", this->framePaddingX ));
	
	if (opt->framePaddingY != this->framePaddingY)
		CPLCreateXMLAttributeAndValue( psTree, "FramePaddingY", CPLString().Printf("%d", this->framePaddingY ));
	
	if (opt->frameTransparency != this->frameTransparency)
		CPLCreateXMLAttributeAndValue( psTree, "FrameTransparency", CPLString().Printf("%d", this->frameTransparency ));
	
	if (opt->frameType != this->frameType)
		CPLCreateXMLAttributeAndValue( psTree, "FrameType", CPLString().Printf("%d", (int)this->frameType ));
	
	if (opt->frameVisible != this->frameVisible)
		CPLCreateXMLAttributeAndValue( psTree, "FrameVisible", CPLString().Printf("%d", (int)this->frameVisible ));

	if (opt->haloColor != this->haloColor)
		CPLCreateXMLAttributeAndValue( psTree, "HaloColor", CPLString().Printf("%d", (int)this->haloColor ));
	
	if (opt->haloSize != this->haloSize)
		CPLCreateXMLAttributeAndValue( psTree, "HaloSize", CPLString().Printf("%d", this->haloSize ));
	
	if (opt->haloVisible != this->haloVisible)
		CPLCreateXMLAttributeAndValue( psTree, "HaloVisible", CPLString().Printf("%d", (int)this->haloVisible ));
	
	if (opt->inboxAlignment != this->inboxAlignment)
		CPLCreateXMLAttributeAndValue( psTree, "InboxAlignment", CPLString().Printf("%d", (int)this->inboxAlignment ));
	
	if (opt->lineOrientation != this->lineOrientation)
		CPLCreateXMLAttributeAndValue( psTree, "LineOrientation", CPLString().Printf("%d", (int)this->lineOrientation ));
	
	if (opt->offsetX != this->offsetX)
		CPLCreateXMLAttributeAndValue( psTree, "OffsetX", CPLString().Printf("%d", (int)this->offsetX ));
	
	if (opt->offsetY != this->offsetY)
		CPLCreateXMLAttributeAndValue( psTree, "OffsetY", CPLString().Printf("%d", (int)this->offsetY ));
	
	if (opt->shadowColor != this->shadowColor)
		CPLCreateXMLAttributeAndValue( psTree, "ShadowColor", CPLString().Printf("%d", (int)this->shadowColor ));
	
	if (opt->shadowOffsetX != this->shadowOffsetX)
		CPLCreateXMLAttributeAndValue( psTree, "ShadowOffsetX", CPLString().Printf("%d", this->shadowOffsetX ));
	
	if (opt->shadowOffsetY != this->shadowOffsetY)
		CPLCreateXMLAttributeAndValue( psTree, "ShadowOffsetY", CPLString().Printf("%d", this->shadowOffsetY ));
	
	if (opt->shadowVisible != this->shadowVisible)
		CPLCreateXMLAttributeAndValue( psTree, "ShadowVisible", CPLString().Printf("%d", this->shadowVisible ));
	
	if (opt->visible != this->visible)
		CPLCreateXMLAttributeAndValue( psTree, "Visible", CPLString().Printf("%d", this->visible ));
