...
bool tkGridRaster::GetIntValueGridColorTable(IGridColorScheme ** newscheme)
{
	...
	{	
		...
		if (poCE != NULL)
			colorEntryExists = true;
		else
			colorEntryExists = false;
...
bool tkGridRaster::LoadRaster(const char * filename, bool InRam, GridFileType fileType)
{
	__try
	{
	   ...
	   //             dX = adfGeoTransform[1];
		...
		if (nWidth != width)
		{
			//AfxMessageBox("width mismatch - updating");
			width = nWidth;
		}
		if (nHeight != height)
		{
			//AfxMessageBox("height mismatch - updating");
			height = nHeight;
		}
		...
		if (success)
			...
			// Write the assumed nodata value (ArCMapView) so that
			// it doesn't die on GetMin.
			poBand->SetNoDataValue(noDataValue);
		...
		{
			// Chris M 1/28/2007 -- multiply this by two, since we use
			// two buffers
