...
bool tkRaster::LoadRaster(const char * filename, GDALAccess accessType)
{
	...
	__try
	{
		...
		if	(ImgType == TIFF_FILE &&  nBands == 1  &&  dataType != GDT_Byte &&  dataType != GDT_UInt16 && !hasColorTable)
			 handleImage = asGrid;
		else if	( dataType == GDT_Byte && !hasColorTable && ImgType != ADF_FILE && ImgType != IMG_FILE && ImgType != KAP_FILE && dfMax > 15)
			 handleImage = asRGB;
		else if (( nBands == 1 || ImgType == ADF_FILE || ImgType == ASC_FILE || ImgType == DEM_FILE) && ImgType != IMG_FILE && ImgType != KAP_FILE && !hasColorTable && dfMax > 1 && !(ImgType == TIFF_FILE &&  dataType == GDT_UInt16)) 
			 handleImage = asGrid;
		else
...
{
	put_ComReference((IDispatch*)scheme, (IDispatch**)&gridColorScheme);
							   ...
							   tkInterpolationMode downsamplingMode, bool setRGBToGrey, double mapUnitsPerScreenPixel)
{
    ...
    else
    {
		...
		// ---------------------------------------------------------
		bool success;
		if ( (gridColorIncoming) && (nBands == 1) ||  (handleImage == asGrid && allowAsGrid))
		{
...
bool tkRaster::ReadGridAsImage(colour** ImageData, int xOff, int yOff, int width, int height, int xBuff, int yBuff, bool setRGBToGrey) 
{
	...
	else if ( imgColor != imageColorScheme )
	{
		CoCreateInstance(CLSID_GridColorScheme,NULL,CLSCTX_INPROC_SERVER,IID_IGridColorScheme,(void**)&gridColorScheme);
		gridColorScheme->UsePredefined(dfMin, dfMax, imageColorScheme);
		imgColor = imageColorScheme;
