...
namespace MapWinGeoProc.Dialogs
{
    ...
    /// </summary>
    public partial class LayerFileElement : UserControl
    {
        #region Variables
        ...
        Image m_HelpImage;
        List<BooleanElement> m_LayerOnlyBoolElements;
        ...
        /// </summary>
        public List<BooleanElement> LayerOnlyBoolElements
        {
            get
            {
                return m_LayerOnlyBoolElements;
            }
            set
            {
                m_LayerOnlyBoolElements = value;
            }
        }
        ...
        public string SelectedLayerName
        {
            ...
            set
            {
                for (int i = 0; i < m_LayerNames.GetUpperBound(0); i++)
                {
                    if (m_LayerNames[i] == value)
                    {
                        m_LayerIndex = i;
                        cbInput.Text = value;
                    }
                }
                // the name wasn't in the list so add it
                string[] tempNames = m_LayerNames;
                m_LayerNames = new string[m_LayerNames.GetUpperBound(0) + 1];
                for (int i = 0; i < m_LayerNames.GetUpperBound(0); i++)
                {
                    m_LayerNames[i] = tempNames[i];
                }
                m_LayerNames[m_LayerNames.GetUpperBound(0)] = value;
            }
        }
        ...
        public int SelectedLayerIndex
        {
            ...
            set
            {
                ...
                {
                    if (m_LayerNames.GetUpperBound(0) > value && value >= 0)
                    {
                        cbInput.Text = m_LayerNames[value];
                        m_LayerName = m_LayerNames[value];
                        return;
                    }
                    if (value < 0)
                    {
                        throw new ArgumentException("Index value can't be negative.");
                    }
                    if (value > m_LayerNames.GetUpperBound(0))
                    {
                        throw new ArgumentException("Index value was greater than the maximum index of LayerNames.");
                    }
                }
                throw new ArgumentException("LayerNames was null.  Specify a LayerNames array before attempting to specify an index.");               
            }
        }
        ...
        /// </summary>
        [Category("Appearance")]
        [Description("Gets or sets the text shown in the combobox for this element.")]
        ...
        public string[] LayerNames
        {
            ...
            set
            {
                ...
                for (int i = 0; i <= m_LayerNames.GetUpperBound(0); i++)
                {
                    cbInput.Items.Add(m_LayerNames[i]);
                }
            }
        }
        ...
        private void cmdBrowse_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (FD.ShowDialog() == DialogResult.OK)
                {
                    cbInput.Text = FD.FileName;
                    this.Status = LightStatus.Ok;
                    if (System.IO.File.Exists(cbInput.Text))
                    {
        ...
        void cbInput_Enter(object sender, System.EventArgs e)
        {
        ...
        private void cbInput_Validating(object sender, EventArgs e)
        {
            if (cbInput.Text == string.Empty)
            {
                LightMessage = "A filename has not been specified.";
                Status = LightStatus.Empty;
                return;
            }
            // Try to match one of the layer names in the list
            int count = m_LayerNames.GetUpperBound(0)+1;
            for (int i = 0; i < count; i++)
            {
                if (cbInput.Text == m_LayerNames[i])
                {
                    m_IsLayer = true;
                    for (int iBE = 0; iBE < m_LayerOnlyBoolElements.Count; iBE++)
                    {
                        m_LayerOnlyBoolElements[iBE].CheckboxEnabled = true;
                    }
                    m_LayerName = m_LayerNames[i];
                    m_LayerIndex = i;
                    LightMessage = "The Layer Name specified is valid.";
            ...
            for (int iBE = 0; iBE < m_LayerOnlyBoolElements.Count; iBE++)
            {
                m_LayerOnlyBoolElements[iBE].CheckboxEnabled = false;
            }
            // The text didn't match any of the layer names
            if (System.IO.File.Exists(cbInput.Text))
            {
                if (m_FileAccess == FileAccessType.Open)
                {
                    string ext = System.IO.Path.GetExtension(cbInput.Text);
