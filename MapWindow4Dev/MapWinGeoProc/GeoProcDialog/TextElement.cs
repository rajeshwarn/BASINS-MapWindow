...
namespace MapWinGeoProc.Dialogs
{
   ...
   /// </summary>
    public partial class TextElement : UserControl
    {
        ...
        TextType m_Type = TextType.TextString;
        double m_MinVal = -9999.0;
        double m_MaxVal = -9999.0;
        ...
        #region Text Element Specific Properties
        /// <summary>
        /// Gets or sets the text type to be allowed for the text field
        /// </summary>
        [Category("Data")]
        [Description("Gets or sets the text type to be allowed for the text field.")]
        ...
        /// </summary>
        [Category("Data")]
        [Description("Gets or sets the value of the text in the text field.")]
        public string Value
        {
            get
            {
        ...
        #region Text Element Specific Event Handling
        
        void tbText_Enter(object sender, System.EventArgs e)
        {
            if (m_HelpButtonVisible == false)
            {
                OnHelpContextChanged(new HelpPannelEventArgs(HelpTitle, HelpText, HelpImage));
            }
        }

        void tbText_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            if (tbText.Text == string.Empty)
            {
                Status = LightStatus.Empty;
                LightMessage = "No value has been set for this element.";
            }
            else
            {
                if (m_Type == TextType.TextInteger)
                {
                    int tmpVal;
                    try
                    {
                        tmpVal = Int32.Parse(tbText.Text);
                        if (m_MinVal != -9999.0)
                        {
                            if (tmpVal < m_MinVal)
                            {
                                Status = LightStatus.Error;
                                LightMessage = "The value entered must be greater than " + m_MinVal.ToString() + ".";
                                return;
                            }
                        }
                        if (m_MaxVal != -9999.0)
                        {
                            ...
                            {
                                Status = LightStatus.Error;
                                LightMessage = "The value entered must be less than " + m_MaxVal.ToString() + ".";
                                return;
                            }
                        }
                        Status = LightStatus.Ok;
                        LightMessage = "The input string is valid.";
                    }
                    catch
                    {
                        Status = LightStatus.Error;
                        LightMessage = "The value entered must be an integer.";
                    }
                }
                else if (m_Type == TextType.TextDouble)
                {
                    Double tmpVal;
                    try
                    {
                        tmpVal = Double.Parse(tbText.Text);
                        if (m_MinVal != -9999.0)
                        {
                            if (tmpVal < m_MinVal)
                            {
                    ...
                    {
                        Status = LightStatus.Error;
                        LightMessage = "The value entered must be a double.";
                    }
                }
                else
                {
