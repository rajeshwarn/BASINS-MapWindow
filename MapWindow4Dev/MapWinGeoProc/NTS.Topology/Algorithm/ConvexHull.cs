...
namespace MapWinGeoProc.NTS.Topology.Algorithm
{
    ...
    public class ConvexHull
    {
        ...
        private ICoordinate[] Reduce(ICoordinate[] pts)
        {
            ...
            var reducedSet = new Iesi.Collections.Generic.SortedSet <ICoordinate>();
        ...
        private Stack<ICoordinate> GrahamScan(ICoordinate[] c)
        {
            ...
            {
                p = (Coordinate)ps.Pop();
                while (CGAlgorithms.ComputeOrientation((ICoordinate)ps.Peek(), p, c[i]) > 0)
                    p = (ICoordinate)ps.Pop();
                ps.Push(p);
                ps.Push(c[i]);
            }
            ps.Push(c[0]);
