...
namespace MapWinGeoProc.NTS.Topology.Algorithm
{
    ...
    public abstract class LineIntersector 
    {
        ...
        /// 
        /// </summary>
        protected ICoordinate[,] inputLines = new Coordinate[2,2];
        
        /// <summary>
        /// 
        /// </summary>
        protected ICoordinate[] intPt = new Coordinate[2];
        ...
        /// </summary>
        public virtual void ComputeIntersection(ICoordinate p1, ICoordinate p2, ICoordinate p3, ICoordinate p4) 
        {
            inputLines[0,0] = new Coordinate(p1);
            inputLines[0,1] = new Coordinate(p2);
            inputLines[1,0] = new Coordinate(p3);
            inputLines[1,1] = new Coordinate(p4);
        ...
        public virtual ICoordinate GetIntersection(int intIndex)  
        { 
            return new Coordinate(intPt[intIndex]); 
        }
        ...
        public virtual bool IsIntersection(ICoordinate pt) 
        {
            for (int i = 0; i < result; i++) 
                if (new Coordinate(intPt[i]).Equals2D(pt)) 
                    return true;                        
            return false;
        }
        ...
        public virtual bool IsInteriorIntersection(int inputLineIndex)
        {
            ...
            for (int i = 0; i < result; i++)
            {
                 ptI = new Coordinate(intPt[i]);
                 if (!(ptI.Equals2D(inputLines[inputLineIndex, 0]) || ptI.Equals2D(inputLines[inputLineIndex, 1])))                                   
                    return true;    
            }
            return false;
        }
        ...
        /// </returns>
        public virtual ICoordinate GetIntersectionAlongSegment(int segmentIndex, int intIndex) 
        {
            // lazily compute int line array
            ComputeIntLineIndex();
            return new Coordinate(intPt[intLineIndex[segmentIndex, intIndex]]);
        }
