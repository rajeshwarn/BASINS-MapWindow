...
namespace MapWinGeoProc.NTS.Topology.Geometries
{
    ...
    public class LineString : Geometry, ILineString
    {
        ...
        /// <param name="n"></param>
        /// <returns></returns>
        public virtual ICoordinate GetCoordinateN(int n) 
        {
        ...
        {
            return new Point(points.GetCoordinate(n));
        }
        ...
        public virtual bool IsCoordinate(ICoordinate pt) 
        {
            Coordinate Coord = new Coordinate(pt);
            for (int i = 0; i < points.Count; i++) 
                if (Coord == points.GetCoordinate(i))
                    return true;                
            return false;
        }
        ...
        public override bool EqualsExact(IGeometry other, double tolerance) 
        {
            ...
            for (int i = 0; i < points.Count; i++)
                if (!Equal(new Coordinate(points.GetCoordinate(i)), otherLineString.points.GetCoordinate(i), tolerance))             
                    return false;            
            return true;
        }
        ...
        #region ILineStringBase Members
        ...
        /// <param name="n"></param>
        /// <returns></returns>
        public ICoordinate GetCoordinateBaseN(int n)
        {
            return points.GetCoordinate(n);
        }
