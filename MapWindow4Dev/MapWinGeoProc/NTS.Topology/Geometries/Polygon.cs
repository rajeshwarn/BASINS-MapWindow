...
namespace MapWinGeoProc.NTS.Topology.Geometries
{
    ...
    public class Polygon : Geometry, IPolygon 
    {
        ...
        public Polygon(IPolygonBase polygonBase) : base(DefaultFactory)
        {
            LinearRing[] holes = new LinearRing[polygonBase.NumInteriorRings];
            ILineStringBase[] BaseHoles = polygonBase.GetInteriorRings();
            for(int I = 0; I < polygonBase.NumInteriorRings; I++)
            {
                holes[I] = new LinearRing(BaseHoles[I]);
            }
            LinearRing shell = new LinearRing(polygonBase.GetExteriorRing());
            if(shell == null)
                Factory.CreateLinearRing((ICoordinateSequence)null);
            if (holes == null)
                holes = new LinearRing[] { };
            if (HasNullElements(holes))
                throw new ArgumentException("holes must not contain null elements");
        ...
        public override ICoordinate[] Coordinates
        {
            get
            {
                ...
                {
                    k++;
                    coordinates[k] = new Coordinate(shellCoordinates[x]);
                }
                for (int i = 0; i < holes.Length; i++)
                {
                    ...
                    {
                        k++;
                        coordinates[k] = new Coordinate(childCoordinates[j]);
                    }
                }
                return coordinates;
            }
        ...
        /// 
        /// </summary>
        public virtual ILineString ExteriorRing
        {
            get
            {
                return shell;
        ...
        /// 
        /// </summary>
        public virtual ILineString[] InteriorRings
        {
            get
            {
                return holes;
        ...
        public override bool EqualsExact(IGeometry other, double tolerance) 
        {
            ...
            if (holes.Length != otherPolygon.holes.Length) 
                return false;
            for (int i = 0; i < holes.Length; i++) 
                if (!((IGeometry) holes[i]).EqualsExact(otherPolygon.holes[i], tolerance)) 
                    return false;
            return true;
        }
        ...
        /// 
        /// </summary>
        public virtual ILinearRing[] Holes
        {
            get
            {
                return holes;
