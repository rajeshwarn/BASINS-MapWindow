...
namespace MapWinGeoProc.Topology
{
    ...
    public class LineSegment
    {
        ...
        public bool Crosses(LineSegment Seg)
        {
            if (Touches(Seg) == true) return false;
            if (Distance(Seg) == 0) return true;
            return false;
        }
        ...
        public double Distance(LineString PolyLn)
        {
            //I'm not sure yet how to do this faster.  Envelope won't really help here.
            List<LineSegment> Segs = PolyLn.ToLineSegments();
            double dist = double.PositiveInfinity;
            for (int iSeg = 0; iSeg < Segs.Count; iSeg++)
            {
                double test = Segs[iSeg].Distance(this);
        ...
        public bool Intersects(LineSegment LineLineSegment)
        {
            if (Distance(LineLineSegment) == 0) return true;
            return false;
        }
        ...
        public bool IsIdenticalTo(LineSegment Seg)
        {
        ...
        public bool IsParallelTo(LineSegment LineLineSegment)
        {
            Vector u = new Vector(this);
            Vector v = new Vector(LineLineSegment);
        ...
        public bool IsPerpendicularTo(LineSegment LineLineSegment)
        {
            Vector u = new Vector(this);
            Vector v = new Vector(LineLineSegment);
        ...
        public bool Touches(LineSegment Seg)
        {
            if (StartPoint.Intersects(Seg)) return true;
            if (EndPoint.Intersects(Seg)) return true;
            if (Seg.StartPoint.Intersects(this)) return true;
            if (Seg.EndPoint.Intersects(this)) return true;
            return false;
        }
