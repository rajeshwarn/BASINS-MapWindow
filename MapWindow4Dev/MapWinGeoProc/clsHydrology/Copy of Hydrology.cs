...
//            }
        ...
        */
        ...
        public static int Mask(string demPath, string maskGridPath, string maskResultPath, ICallback callback)
        {
            ...
            double maskHeadNodataValue = double.Parse(maskHead.NodataValue.ToString());
            ...
            if (SpatialOperations.ClipGridWithPolygon(ref demPath, ref tmpShape, ref maskResultPath))
            {
                ...
                for (var row = 0; row <= maskHead.NumberRows - 1; row++)
                {
                    ...
                    {
                        if (callback == null)
                            MapWinUtility.Logger.Progress(row, maskHead.NumberRows);
                        else
                        {
                            var newperc = row * 100 / maskHead.NumberRows;
                            if (newperc > oldperc)
                            {
                    ...
                    for (var col = 0; col <= maskHead.NumberCols - 1; col++)
                    {
                        ...
                        if (Convert.ToDouble(g.get_Value(col, row)) == maskHeadNodataValue)
                        {
...
//                }
        ...
        //private static double GridDouble(Grid grid, int Column, int Row)
        //{
        //    switch (grid.DataType)
        //    {
        //        case GridDataType.DoubleDataType: return (double)(grid.get_Value(Column, Row));
        //        case GridDataType.ByteDataType: return (byte)(grid.get_Value(Column, Row));
        //        case GridDataType.FloatDataType: return (float)(grid.get_Value(Column, Row));
        //        case GridDataType.LongDataType: return (long)(grid.get_Value(Column, Row));
        //        case GridDataType.ShortDataType: return (short)(grid.get_Value(Column, Row));
        //        default: return double.Parse(grid.get_Value(Column, Row).ToString());
        //    }
        //}
        ...
        public static int CanyonBurnin(string streamNetShapePath, string demPath, string burnedDemResultPath, ICallback callback)
        {
            ...
            int NumberCols = gridDem.Header.NumberCols;

            gridTrack = new BitArray(NumberCols * gridDem.Header.NumberRows);// Boolean[gridDem.Header.NumberCols, gridDem.Header.NumberRows];
            ...
            for (i = 0; i < lineShape.NumShapes; i++)
            {
                ...
                {
                    int j;
                    int lastPoint = tmpShape.numPoints - 1;
                    for (j = 1; j <= lastPoint; j++)
                    {
                        int x0;
                        int y0;
                        ...
                        for (x = x0; x <= x1; x++)
                        {
                            ...
                            if (steep)
                            {
                                if (!gridTrack[y * NumberCols + x])
                                {
                                    currVal = Convert.ToDouble(gridDem.get_Value(y, x));
                                    if (nodata < 0)
                                    {
                                        if (currVal > nodata)
                                        {
                                            gridTrack[y * NumberCols + x] = true;
                                            if (Subval)
                                            {
                            ...
                            {
                                if (!gridTrack[x * NumberCols + y])
                                {
                                    currVal = Convert.ToDouble(gridDem.get_Value(x, y));
                                    if (nodata < 0)
                                    {
                                        if (currVal > nodata)
                                        {
                                            gridTrack[x * NumberCols + y] = true;
                                            if (Subval)
                                            {
...
//            }
            ...
            for (var sindx = 0; sindx < shedShape.NumShapes; sindx++)
            {
                ...
                for (row = 0; row < numberRows; row += 2)
                {
                    ...
                    {
                        var currVal = Convert.ToDouble(tmpClipGrid.get_Value(col, row));
...
//            }
        ...
        public static bool ApplyWatershedSlopeAttribute(string subBasinGridPath, string subBasinShapePath, string slopeGridPath, ElevationUnits elevUnits, ICallback callback)
        {
            ...
            for (row = 0; row < numberRows; row++)
            {
                ...
                {
                    var currVal = Convert.ToDouble(slopeGrid.get_Value(col, row));
                    ...
                    var currBasinID = Convert.ToInt32(subBasinGrid.get_Value(col, row));
...
//            }
            ...
            // EditingTable can remain true
            if (newShed.EditingTable) newShed.StopEditingTable(true, null);
            newShed.Close();
...
//            }
            ...
            for (sindx = 0; sindx <= shedShape.NumShapes - 1; sindx++)
            {
                ...
                if (SpatialOperations.ClipGridWithPolygon(ref ElevGridPath, ref tmpPoly, ref tmpClipPath))
                {
                    ...
                    for (row = 0; row <= nr - 1; row += 2)
                    {
                        ...
                        {
                            currVal = Convert.ToDouble(tmpClipGrid.get_Value(col, row));
                            if (currVal != nodataVal)
                            {
        ...
        public static bool ApplyWatershedElevationAttribute(string subBasinGridPath, string subBasinShapePath, string elevGridPath, ICallback callback)
        {
            ...
            for (var row = 0; row < numberRows; row++)
            {
                ...
                {
                    var currVal = Convert.ToDouble(elevGrid.get_Value(col, row));
...
//            }
        ...
        private static void GetStreamElevationPoints(int sindx, Shapefile streamShape, Grid demGrid, out double elevLow, out double elevHigh)
        {
            ...
            for (var i = 0; i < shapePoints; i += 2)
            {
                ...
                var currVal = Convert.ToDouble(demGrid.get_Value(currCol, currRow));
                if (currVal < elevLow)
                {
                    elevLow = currVal;
                }
