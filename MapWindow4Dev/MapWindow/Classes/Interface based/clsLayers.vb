...
#Region "Properties"
    ...
    Public Property CurrentLayer() As Integer Implements Interfaces.Layers.CurrentLayer
        Get
            If frmMain.IsDisposed Then Return Nothing
...
#Region "Add layer"
    ...
    Friend Function AddLayer(Optional ByVal ObjectOrFilename As Object = "", _
        ...
        Else
            ...
            Else
                ...
                Dim addingSession As Boolean = m_mismatchTester IsNot Nothing
                Try
                    ...
                    If Not tileLayer Then
                        If source.Projection.IsEmpty Then
                            Try
                                Dim lPrjFileName As String
                                Dim lProjection As New MapWinGIS.GeoProjectionClass()
                                If ProjectionMatchesProject(source.Filename) Then
                                    lProjection.ImportFromProj4(modMain.ProjInfo.ProjectProjection)
                                End If
                                If lProjection.IsEmpty OrElse String.IsNullOrEmpty(lProjection.ExportToProj4) Then
                                    lPrjFileName = IO.Path.ChangeExtension(source.Filename, ".prj")
                                    If IO.File.Exists(lPrjFileName) Then
                                        lProjection.ImportFromESRI(IO.File.ReadAllText(lPrjFileName))
                                    End If
                                End If
                                If lProjection.IsEmpty OrElse String.IsNullOrEmpty(lProjection.ExportToProj4) Then
                                    lPrjFileName = IO.Path.ChangeExtension(source.Filename, ".proj4")
                                    If IO.File.Exists(lPrjFileName) Then
                                        Dim lInputProj4 As String = IO.File.ReadAllText(lPrjFileName)
                                        lProjection.ImportFromProj4(lInputProj4)
                                    End If
                                End If
                                If Not lProjection.IsEmpty Then
                                    source.Projection = lProjection
                                End If
                            Catch
                            End Try
                        End If
                        If Not ProjectionMatchesProject(source.Filename) Then
    ...
    Private Function ProjectionMatchesProject(ByVal aFileName As String) As Boolean
        Try
            Dim lPrjFileName As String = IO.Path.ChangeExtension(aFileName, ".prj")
            If IO.File.Exists(lPrjFileName) Then
                Dim lInputProjection As New MapWinGIS.GeoProjection
                lInputProjection.ImportFromESRI(IO.File.ReadAllText(lPrjFileName))
                Dim lInputProj4 As String = lInputProjection.ExportToProj4()

                Dim lOutputProjection As New MapWinGIS.GeoProjection
                lOutputProjection.ImportFromProj4(modMain.ProjInfo.ProjectProjection)
                Dim lOutputProj4 As String = lOutputProjection.ExportToProj4

                If clsProjections.PartsCompare(lInputProj4, lOutputProj4, True) OrElse _
                    lInputProjection.ExportToWKT().Equals(lOutputProjection.ExportToWKT()) Then
                    Return True
                End If
                Try 'See if 1,1 projects to the same lat/lon in layer and current map projections
                    Dim x1 As Double = 1, y1 As Double = 1
                    Dim x2 As Double = 1, y2 As Double = 1
                    If MapWinGeoProc.SpatialReference.ProjectPoint(x1, y1, modMain.ProjInfo.ProjectProjection, "+proj=longlat +datum=NAD83") AndAlso _
                        MapWinGeoProc.SpatialReference.ProjectPoint(x2, y2, lInputProj4, "+proj=longlat +datum=NAD83") Then
                        If Math.Abs(x1 - x2) < 0.01 AndAlso Math.Abs(y1 - y2) < 0.01 Then
                            Return True
                        End If
                    End If
                Catch
    ...
    Private Function AddLayerFromFilename(ByVal filenames() As String, _
        ...
        Try
            ...
            For i As Integer = 0 To filenames.Length - 1
                ...
                Dim result As TestingResult = TestingResult.Error
                If ProjectionMatchesProject(filenames(i)) Then
                    result = TestingResult.Ok
                Else
                    Try
                        result = m_mismatchTester.TestLayer(filenames(i), newName)
                    Catch ex As Exception
                        MapWinUtility.Logger.Dbg("TestProjection(" & filenames(i) & "): Exception testing projection: " & ex.ToString)
                    End Try
                End If
            ...
            Select Case AppInfo.SymbologyLoadingBehavior
                ...
                Case SymbologyBehavior.DefaultOptions
                    If newLyr.LayerType = Interfaces.eLayerType.LineShapefile Or _
                        ...
                        ' first we'll try .mwsymb with default name (.shp.mwsymb and .shp.view-default.mwsymb)
...
#Region "Grid utilities"
    ...
    Public Sub GetImageRep(ByVal filename As String, ByRef newImage As MapWinGIS.Image, ByRef newGrid As MapWinGIS.Grid, ByRef GrdColorScheme As MapWinGIS.GridColorScheme, ByRef cb As MapWinGIS.ICallback)
        Try
            ...
            imageTransparencyValue = CType(ReadLayerPropertyValueFromXML(xmlfilename, "GridColoringScheme", "ImageLayerFillTransparency", 1), Single)
            If imageTransparencyValue > 1 Then
                newImage.TransparencyPercent = imageTransparencyValue
            Else
                newImage.TransparencyPercent = imageTransparencyValue * 100
            End If

            ' Image Downsampling method
            imageDownsamplingMethod = ReadLayerPropertyValueFromXML(xmlfilename, "GridColoringScheme", "ImageDownsamplingMethod", "None")
            ...
            imageUpsamplingMethod = ReadLayerPropertyValueFromXML(xmlfilename, "GridColoringScheme", "ImageUpsamplingMethod", "None")
