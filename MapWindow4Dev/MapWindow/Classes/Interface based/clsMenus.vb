...
'1/31/2008 - changed declaration of ResourceManager in EnsureHelpItemLast() function
...
Public Class Menus
    ...
    Default Public ReadOnly Property Item(ByVal MenuName As String) As MapWindow.Interfaces.MenuItem Implements Interfaces.Menus.Item
        Get
            ...
            If m_MenuTable.ContainsKey(key) Then
                ...
                If Not pluginfile.ToLower().EndsWith("mapwindow.exe") Then
                    ...
                    If info.Init(pluginfile, GetType(Interfaces.IPlugin).GUID) AndAlso Not info.Key = "" Then
                        pluginkey = info.Key
                    ElseIf info.Init(pluginfile, GetType(MapWindow.PluginInterfaces.IBasePlugin).GUID) AndAlso Not info.Key = "" Then
                        pluginkey = info.Key
                    End If
                    If Not pluginkey = "" Then MenuTrackerRemove(MenuName, pluginkey)
                End If

                ' Paul Meems 15 sept 2009, Bug 1412
                ' If all menu-items are removed remove the menustrip as well:
                If (frmMain.MenuStrip1.Items.Count = 0) Then
                    frmMain.StripDocker.TopToolStripPanel.Controls.Remove(frmMain.MenuStrip1)
    ...
    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String) As MapWindow.Interfaces.MenuItem Implements Interfaces.Menus.AddMenu
        'AddMenu Type 1 Adds a menu item to the main menu based on a name only.  
        Return AddMenu(MenuName, "", Nothing, MenuName, "")
    End Function

    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal Picture As Object) As MapWindow.Interfaces.MenuItem Implements Interfaces.Menus.AddMenu
        'AddMenu Type 2 Adds a menu item to the main menu based on a name and a picture. 
        Return AddMenu(MenuName, "", Picture, MenuName, "")
    End Function

    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal Picture As Object, ByVal Text As String) As MapWindow.Interfaces.MenuItem Implements MapWindow.Interfaces.Menus.AddMenu
        'AddMenu Type 3 Adds a menu item to the main menu using name, text and picture. 
        Return AddMenu(MenuName, "", Picture, Text, "")
    End Function

    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal ParentMenu As String, ByVal Picture As Object) As MapWindow.Interfaces.MenuItem Implements MapWindow.Interfaces.Menus.AddMenu
    ...
    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal ParentMenu As String, ByVal Picture As Object, ByVal [Text] As String) As MapWindow.Interfaces.MenuItem Implements MapWindow.Interfaces.Menus.AddMenu
    ...
    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal ParentMenu As String) As MapWindow.Interfaces.MenuItem Implements MapWindow.Interfaces.Menus.AddMenu
    ...
    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal ParentMenu As String, ByVal Text As String, ByVal Before As String) As MapWindow.Interfaces.MenuItem Implements MapWindow.Interfaces.Menus.AddMenu
        ...
        If (String.IsNullOrEmpty(Text) AndAlso Not String.IsNullOrEmpty(Before)) Then
    ...
    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal ParentMenu As String, ByVal Picture As Object, ByVal Text As String, ByVal After As String) As MapWindow.Interfaces.MenuItem Implements MapWindow.Interfaces.Menus.AddMenu
        'AddMenu Type 4
    ...
    <CLSCompliant(False)> _
    Public Function AddMenu(ByVal MenuName As String, ByVal ParentMenu As String, ByVal Picture As Object, ByVal Text As String, ByVal After As String, ByVal Before As String) As MapWindow.Interfaces.MenuItem
        ...
        'If Text.EndsWith("19") Then Stop
        Try
            If String.IsNullOrEmpty(MenuName) OrElse MenuName.Trim.Length = 0 Then
            ...
            If NewMenu Is Nothing Then
                ...
                ' Paul Meems 15 sept 2009
                ' Make sure the menu is visible:
                frmMain.MenuStrip1.Visible = True

                If String.IsNullOrEmpty(ParentMenu) OrElse ParentMenu.Trim.Length = 0 Then
                ...
                Else
                    ...
                    If Not Parent Is Nothing Then
                        ...
                        NewMenu.Name = MenuName
                        If Parent.Name = "mnuRecentProjects" Then
                            Debug.WriteLine("AddMenu: " & InsertPosition & ": " & NewMenu.Text & " (" & NewMenu.Name & ")")
                        End If
                        'No other way to get the event handler than the above; no useful overloads
                        'So now remove it and readd it where it's supposed to be.
                        If InsertPosition < Parent.DropDownItems.Count - 1 Then
            ...
            Dim newItem As MapWindow.Interfaces.MenuItem
            If TypeOf (NewMenu) Is ToolStripSeparator Then
    ...
    'ResourceManager declaration changed by Jiri Kadlec 1/31/2008
    Public Sub EnsureHelpItemLast()
    ...
    ''' <summary>
    ''' Activates the menu item as it is clicked by the mouse.
    ''' </summary>
    ''' <param name="menuName">The name of the menu</param>
    ''' <returns>True on success</returns>
    ''' <remarks>Added by Paul Meems on 19 Aug 2010</remarks>
    Public Function PerformClick(ByVal menuName As String) As Boolean Implements Interfaces.Menus.PerformClick
        Try
            Dim menuItem As Windows.Forms.ToolStripItem = MenuTableItem(menuName)
            If menuItem Is Nothing Then Return False

            ' Do the click like a mouse click
            menuItem.PerformClick()

            Return True

        Catch ex As Exception
            MapWinUtility.Logger.Dbg("Error in clsMenu.PerformClick(" & menuName & "): " & ex.Message)
        End Try
    ...
    Public Function MenuNames() As System.Collections.Generic.IList(Of String) Implements Interfaces.Menus.MenuNames
        Dim menuNamesList As System.Collections.Generic.IList(Of String) = New System.Collections.Generic.List(Of String)

        For Each de As DictionaryEntry In m_MenuTable
            menuNamesList.Add(de.Key)
        Next de
    ...
    ''' <returns>True on success</returns>
    ''' <remarks>Added by Paul Meems on May 4, 2011</remarks>
    Public Function RemoveAllMenus() As Boolean Implements Interfaces.Menus.RemoveAllMenus
        Try
            'loop through all existing toolbars
            Dim allMenus As System.Collections.Generic.List(Of String) = MenuNames()

            For Each menuName As String In allMenus
                Remove(menuName)
            Next menuName

            Return True
        Catch ex As Exception
            MapWinUtility.Logger.Dbg("Error in RemoveAllMenus: " + ex.Message)
