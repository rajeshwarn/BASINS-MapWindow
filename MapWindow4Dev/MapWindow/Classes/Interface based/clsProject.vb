    ...
    Public Function GetProjectionFromUser() As MapWinGIS.GeoProjection
        ...
        If (form.ShowDialog(modMain.frmMain) = Windows.Forms.DialogResult.OK) Then
            ...
            If lCoordinateSystem IsNot Nothing Then
                proj = New MapWinGIS.GeoProjection()
                If proj.ImportFromEPSG(lCoordinateSystem.Code) Then
                    If Not frmMain.ApplicationInfo.FavoriteProjections.Contains(lCoordinateSystem.Code) Then
                        frmMain.ApplicationInfo.FavoriteProjections.Add(lCoordinateSystem.Code)
                    End If
                Else
                    proj = Nothing
                    MessageBox.Show("Failed to initialize the selected projection" & vbCrLf & lCoordinateSystem.Code & " " & lCoordinateSystem.Name, modMain.AppInfo.Name, MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
            Else
                MessageBox.Show("Selected projection not found", modMain.AppInfo.Name, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End If
