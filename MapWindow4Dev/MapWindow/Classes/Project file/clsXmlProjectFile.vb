...
#Region "Load Project File"
    ...
    Private Function LoadMW4Settings(ByVal node As XmlElement, ByVal LayersOnly As Boolean) As Boolean
        ...
        If Not LayersOnly Then
            ...
            If node.HasAttribute("ProjectProjectionWKT") AndAlso proj.ImportFromWKT(node.Attributes("ProjectProjectionWKT").InnerText) Then
                'Using ProjectProjectionWKT
            ElseIf node.HasAttribute("ProjectProjection") AndAlso proj.ImportFromProj4(node.Attributes("ProjectProjection").InnerText) Then
                'Using ProjectProjection as Proj4
            ElseIf node.HasAttribute("ProjectProjection") AndAlso proj.ImportFromWKT(node.Attributes("ProjectProjection").InnerText) Then
                'Using ProjectProjection as WKT
            Else
                MapWinUtility.Logger.Dbg("ProjectProjection could not be read")
                Return False
            End If
...
#Region "Loading"
    ...
    Private Function LoadView(ByVal view As XmlElement) As Boolean
        Try
            ...
            SetFromAttributeBool(AppInfo.LabelsUseProjectLevel, view.Attributes, "LabelsUseProjectLevel", False)

            If frmMain.WindowState = FormWindowState.Minimized Then
                frmMain.WindowState = FormWindowState.Normal
            End If

            SetFromAttributeBool(TransparentSelection, view.Attributes, "TransparentSelection", True)

            ' legend panel visiblity
            Dim visible As Boolean = True
            SetFromAttributeBool(visible, view.Attributes, "LegendVisible")
            frmMain.UpdateLegendPanel(visible)

            ' preview visiblity
            SetFromAttributeBool(visible, view.Attributes, "PreviewVisible", True)
...
#Region "Load Layers Functions"
    ...
    Friend Sub LoadLayerProperties(ByVal layer As XmlNode, Optional ByVal ExistingLayerHandle As Integer = -1, Optional ByVal PluginCall As Boolean = False)
        ...
        Try
            ...
            Catch
            End Try
            Dim layerVisible As Boolean = False
            Dim type As Integer = 0
            Dim expanded As Boolean = False
            Dim tag As String = layer.Attributes("Tag").InnerText
            Dim labelsVisible As Boolean = False
            Dim handle As Integer
            Dim imageType As String
            Dim gridScheme As MapWinGIS.GridColorScheme = Nothing

            SetFromAttributeBool(layerVisible, layer.Attributes, "Visible")
            SetFromAttributeInt(type, layer.Attributes, "Type")
            SetFromAttributeBool(expanded, layer.Attributes, "Expanded")
            SetFromAttributeBool(labelsVisible, layer.Attributes, "LabelsVisible")
            ...
            If type = MapWindow.Interfaces.eLayerType.LineShapefile Or type = MapWindow.Interfaces.eLayerType.PointShapefile Or type = MapWindow.Interfaces.eLayerType.PolygonShapefile Then
                ...
                With layer.Item("ShapeFileProperties")
                    ...
                    SetFromAttributeBool(fillStippleTransparent, .Attributes, "FillStippleTransparent")
                    ...
                    If (Not userPointType Is Nothing) Then
                        SetFromAttributeBool(userPointType.UseTransparencyColor, .Attributes, "UseTransparency", False)
                    ...
                    Else
                        SetFromAttributeBool(frmMain.Layers(handle).VerticesVisible, .Attributes, "VerticesVisible", False)
                    End If
                    SetFromAttributeBool(frmMain.Layers(handle).LabelsVisible, .Attributes, "LabelsVisible", True)

                    SetFromAttributeInt(frmMain.Legend.Layers.ItemByHandle(handle).MapTooltipFieldIndex, .Attributes, "MapTooltipField")
                    SetFromAttributeBool(frmMain.Legend.Layers.ItemByHandle(handle).MapTooltipsEnabled, .Attributes, "MapTooltipsEnabled")
            ...
            ElseIf type = MapWindow.Interfaces.eLayerType.Image Or type = MapWindow.Interfaces.eLayerType.Grid Then
                ...
                If GridProperty IsNot Nothing Then
                    With GridProperty
                        Try
                            SetFromAttributeInt(transparentColor, .Attributes, "TransparentColor")
                            ' Start Paul Meems May 25 2010, Issue 1714
                            ' Old situation:
                        ...
                        Try
                            ' New situation:
                            SetFromAttributeInt(transparentColor, .Attributes, "TransparentColor")
                            SetFromAttributeInt(transparentColor2, .Attributes, "TransparentColor2")
                        Catch
                        End Try
                        ' End Paul Meems May 25 2010, Issue 1714
                        Try
                            SetFromAttributeBool(useTransparency, .Attributes, "UseTransparency")
                        Catch
                        End Try
                        Try
                        ...
                        End Try
                        Try
                            SetFromAttributeBool(useHistogram, .Attributes, "UseHistogram")
                        Catch
                        End Try
                        Try
                            SetFromAttributeBool(allowHillshade, .Attributes, "AllowHillshade")
                        Catch
                        End Try
                        Try
                            SetFromAttributeBool(setToGrey, .Attributes, "SetToGrey")
                        Catch
                        End Try
                        Try
                            SetFromAttributeInt(bufferSize, .Attributes, "BufferSize")
                        Catch
                        End Try
                        Try
                            Dim cs As String
    ...
    Private Function SetFromAttributeBool(ByRef aSetMe As Boolean, _
                                          ByVal aAttributes As System.Xml.XmlAttributeCollection, _
                                          ByVal aAttributeName As String)
        Dim lAttribute As System.Xml.XmlAttribute = aAttributes(aAttributeName)
        Return lAttribute IsNot Nothing AndAlso Boolean.TryParse(lAttribute.InnerText, aSetMe)
    End Function

    Private Sub SetFromAttributeBool(ByRef aSetMe As Boolean, _
                                     ByVal aAttributes As System.Xml.XmlAttributeCollection, _
                                     ByVal aAttributeName As String, _
                                     ByVal aDefaultValue As Boolean)
        If Not SetFromAttributeBool(aSetMe, aAttributes, aAttributeName) Then
            aSetMe = aDefaultValue
        End If
    End Sub

    Private Function SetFromAttributeInt(ByRef aSetMe As Integer, _
                                    ByVal aAttributes As System.Xml.XmlAttributeCollection, _
                                    ByVal aAttributeName As String)
        Dim lAttribute As System.Xml.XmlAttribute = aAttributes(aAttributeName)
        Return lAttribute IsNot Nothing AndAlso Integer.TryParse(lAttribute.InnerText, aSetMe)
    End Function

    Private Sub SetFromAttributeInt(ByRef aSetMe As Integer, _
                                    ByVal aAttributes As System.Xml.XmlAttributeCollection, _
                                    ByVal aAttributeName As String, _
                                    ByVal aDefaultValue As Integer)
        If Not SetFromAttributeInt(aSetMe, aAttributes, aAttributeName) Then
            aSetMe = aDefaultValue
        End If
    End Sub
