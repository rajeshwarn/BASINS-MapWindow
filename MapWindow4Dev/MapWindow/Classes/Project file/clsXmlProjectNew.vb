    ...
    Friend Sub LoadLayers(ByRef nodeOcx As XmlNode, ByRef nodeMW4 As XmlNode, Optional ByVal ExistingLayerHandle As Integer = -1, Optional ByVal PluginCall As Boolean = False)
        ...
        For i As Integer = 0 To layerCount
            Try
                    ...
                    If nodeLayer.HasAttribute("Filename") Then
                        ...
                        If nodeLayer.HasAttribute("GridFilename") Then
                            Try
                                ...
                                If nodeScheme IsNot Nothing Then
                                    ...
                                    Dim layArray As Interfaces.Layer() = frmMain.m_layers.AddLayer(gridName, name, GrdColorScheme:=scheme, LoadXMLInfo:=False)
                                    If layArray Is Nothing OrElse layArray.Length = 0 OrElse layArray(0) Is Nothing Then
                                        handle = -1
                                    Else
                                        handle = layArray(0).Handle
                                    End If
                                End If
                            Catch exGrid As Exception
                                MapWinUtility.Logger.Dbg("Exception loading grid scheme: " & exGrid.ToString)
                            End Try
                        End If
                        If handle = -1 Then
                ...
                If nodeLegend IsNot Nothing Then
                ...
                If Not nodeMeta Is Nothing Then
                    ...
                    End If
                End If
            Catch exLoadLayer As Exception
                If String.IsNullOrEmpty(filepath) Then
                    lErrorMessage = "Could not load layer " & i & " - " & exLoadLayer.Message
                Else
                    lErrorMessage = "Could not load '" & filepath & "' - " & exLoadLayer.Message
                End If
                Select Case MapWinUtility.Logger.MsgCustom(lErrorMessage, "Load Project", "Ok", "Retry", "Abort")
                    Case "Retry" : i -= 1
                    Case "Abort" : Exit For
                End Select
            End Try
