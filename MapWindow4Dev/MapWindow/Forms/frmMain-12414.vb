...
#Region "Declarations"
    ...
    Friend WithEvents StatusBarPanelScale As System.Windows.Forms.StatusBarPanel
...
#Region "Initialization"
    ...
    Public Function CreateLegendPanel() As WeifenLuo.WinFormsUI.Docking.DockContent
        ...
        ' test code: displaying toolbar above the legend
        ' -------------------------------------------------- 
        'Dim s As String
        'Dim strip As ToolStrip = New ToolStrip()
        'strip.Dock = DockStyle.Top

        's = mnuLegend.Items("ToolStripMenuItem2").Text
        'strip.Items.Add("", GlobalResource.imgGroupAdd, New System.EventHandler(AddressOf ToolStripMenuItem2_Click))
        'strip.Items(0).ToolTipText = s
        'legendPanel.Controls.Add(strip)
    ...
    Friend Sub LoadToolStripSettings(ByVal toolStripContainer As ToolStripContainer)
        ...
        For Each panel As Control In toolStripContainer.Controls
            ...
            For Each myControl As Control In myToolstripPanel.Controls
                ...
                Dim toolStripSettings = New ToolStripSettings(myToolstrip.Name)
                Dim location = toolStripSettings.Location
                Dim displayStyle = toolStripSettings.DisplayStyle
                ...
                ' change that as well. This was first not necessary:
                ...
                ' Change location:
                If location.X > -1 Then
                    ' check for unnessecary moving:
                    If myToolstrip.Location.X <> location.x OrElse myToolstrip.Location.Y <> location.y Then
                        myToolstrip.Location = location
                    End If
                End If
            Next

            ' The toolstrips keep randomly placed in the top panel,
            ' I can't figure out why. As a last resort I'm going to check all locations a few times
            ' hoping to get it right this time:
            For i As Integer = 0 To 10
                For Each myControl As Control In toolStripContainer.TopToolStripPanel.Controls
                    ...
                    Dim location = toolStripSettings.Location
                    If location.X = -1 Then
                        ' No settings found skip all
                        'MsgBox("No settings found, skip all")
                        i = 11
                        Exit For
                    End If
                    ...
                    If myToolstrip.Location.X <> location.x OrElse myToolstrip.Location.Y <> location.y Then
                        ...
                        myToolstrip.Location = location
                        'MsgBox(String.Format("The location of toolstrip {0} was changed to {1}, {2}", myToolstrip.Name, location.x, location.y), MsgBoxStyle.OkOnly, "Location changed")
                        ' It is working when calling a MsgBox, without it it doesn't
                        ' So let's try DoEvents and Sleep:
        ...
        ' In some cases the menu strip has been moved below or between the toolstrips.
        ' I have no idea way. In any case set the menustrip to the correct location:
    ...
    Public Sub SetUpMenus()
        ...
        m_Menu.AddMenu("mnuAddECWPLayer", "mnuView", resources.GetObject("imgLayerAdd"), resources.GetString("mnuAddECWPLayer.Text"))
        ...
        'm_Menu.AddMenu("mnuBookmarkDelete", "mnuBookmarks", resources.GetObject("imgBookmarkDelete"), resources.GetString("mnuBookmarkDelete.Text"))
        m_Menu.AddMenu("mnuBookmarkedViews", "mnuBookmarks", resources.GetObject("imgBookmarkView"), resources.GetString("mnuBookmarkedViews.Text"))
        'Christian Degrassi 2010-03-02: This fixes part of issue 1364
        m_Menu.AddMenu("mnuBookmarksBreak1", "mnuBookmarks", Nil, "-")
        m_Menu.AddMenu("mnuBookmarksManager", "mnuBookmarks", resources.GetObject("imgBookmarksManager"), resources.GetString("mnuBookmarksManager.Text"))

        'Christian Degrassi 2010-03-22: Layers Menu. Preparing for Enhancement 1651
        'Set up the Layers menu
        'Dim LayersMenu As MenuItem
        'LayersMenu = m_Menu.AddMenu("mnuLayers", Nil, resources.GetString("mnuLayers.Text"))
        '_LayersMenuItemFactory = New LayersMenuItemFactory(New EventHandler(AddressOf Me.CustomMenu_Click))
        '_LayersMenuItemFactory.CreateCommonSubItems(CType(LayersMenu, ToolStripMenuItem).ToObject)
        '_LayersMenuItemFactory.UpdateMenuForLayer(Nothing)
...
#Region "MapWindowForm Events"
    ...
    Private Function HandleShortcutKeys(ByVal e As Keys) As Boolean
        ...
        'This doesn't do anything so I commented it:
        ...
        MapWinUtility.Logger.Dbg("CurrentExtent: " + m_CurrentExtent.ToString())
...
#Region "Legend Events"
    ...
    Private Sub Legend_LayerSelected(ByVal Handle As Integer) Handles Legend.LayerSelected
        ...
        '3 Nov. 2010 Paul Meems - No need to continue when the layer is invalid:
        If Handle = -1 Then Return

        Try
            Dim lastMode As MapWinGIS.tkCursorMode = MapMain.CursorMode

            ' lsu: don't see reason to clear selection for the new ocx
    ...
    Private Sub ToolStripMenuItem16_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem16.Click
        ...
        'If (MapMain.ShapeDrawingMethod <> MapWinGIS.tkShapeDrawingMethod.dmNewSymbology) Then
        'If mnuLegend.ModifierKeys <> Keys.Shift Then
        DoEditProperties()
        'ElseIf (MapMain.ShapeDrawingMethod = MapWinGIS.tkShapeDrawingMethod.dmNewSymbology) Then
        ''  opening new symbology plug-in
        'Plugins.BroadcastMessage("SYMBOLOGY_EDIT" + Convert.ToString(Legend.SelectedLayer))
        'End If
    End Sub
...
#Region "MapMain Events"
    ...
    Private Sub MapMain_MouseUpEvent(ByVal sender As System.Object, ByVal e As AxMapWinGIS._DMapEvents_MouseUpEvent) Handles MapMain.MouseUpEvent
        ...
        ElseIf tbbMeasureArea.Checked Then
            ...
            Else
                ...
                Dim resultArea As String = AppInfo.AreaMeasuringCalculate()
    ...
    Private Sub MapMain_ExtentsChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MapMain.ExtentsChanged
        ...
        Dim scalePanel As StatusBarPanel = Me.StatusBar1.Panels("StatusBarPanelScale")
        scalePanel.Text = "1: " + Math.Round(Double.Parse(GetCurrentScale(), CultureInfo.InvariantCulture)).ToString(CultureInfo.InvariantCulture)
...
#Region "Toolbar Functions"
    ...
    Friend Sub UpdateButtons()
        ...
        'tlbMain:
        tbbMeasure.Enabled = (numLayers > 0)
        tbbMeasureArea.Enabled = (numLayers > 0)
        tbbSelect.Enabled = (numLayers > 0)
        ...
        'Zoom menu items, are thes still used?
        ...
        tbbLayerSaveAs.Enabled = (numLayers > 0)
        ...
        Select Case BtnName
            ...
            Case "tbbMeasure"
                If Not AppInfo.MeasuringCurrently Then
                    AppInfo.MeasuringBegin()
                Else
                    AppInfo.MeasuringStop()
                End If
                UpdateButtons()
                'Notify plugins - since this is done after handing it locally,
                'they can't override it -- but they do notice it.
    ...
    Private Sub tlbZoom_ItemClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tlbZoom.ItemClicked
        ...
        Dim BtnName As String = CType(Btn.Name, String)
        If BtnName.Trim() = "" And TypeOf (Btn.Tag) Is String Then BtnName = CStr(Btn.Tag)
        HandleZoomButtonClick(BtnName, Btn)
    End Sub
    ...
    Friend Sub HandleZoomButtonClick(ByVal BtnName As String, ByVal Btn As ToolStripButton)
        If AppInfo.MeasuringCurrently And Not BtnName = "tbbMeasure" Then AppInfo.MeasuringStop()
        If AppInfo.AreaMeasuringCurrently And Not BtnName = "tbbMeasureArea" Then AppInfo.AreaMeasuringStop()
...
#Region "General Functions"
    ...
    Private Sub DoOpen()
        ...
        If System.IO.File.Exists(cdlOpen.FileName) Then
            ...
            If System.IO.Path.GetExtension(cdlOpen.FileName) = ".mwprj" Then
                ...
                If Not ProjInfo.LoadProject(cdlOpen.FileName) Then
                    ' Paul Meems 10 Aug 2010: moved error box to here from LoadProject():
                    ' TODO Needs localization:
                    MapWinUtility.Logger.Msg("Errors occured while opening this project file", MsgBoxStyle.Exclamation, "Project File Error Report")
                End If
    ...
    Private Sub DoAddLayer()
        ...
        End If
    End Sub
    Private Sub DoSaveAsLayer()
        If (Legend.SelectedLayer <> -1) Then

            Dim lyr As Layer = Layers(Legend.SelectedLayer)
            Dim dlg As New SaveFileDialog()
            dlg.Filter = "MapWindow Layers (*.mwlyr)|*.mwlyr"
            dlg.FilterIndex = 0
            If (dlg.ShowDialog() = Windows.Forms.DialogResult.OK) Then
                Dim layerFile As New clsXMLLayerFile
                layerFile.SaveLayerFile(dlg.FileName, lyr)
            End If
    ...
    Public Function GetProjectionFromUser(ByVal DialogCaption As String, ByVal DefaultProjection As String) As String Implements Interfaces.IMapWin.GetProjectionFromUser
        Dim newProjections As New MapWinGeoProc.Projections.Projections
        ...
        'Return retval
        'retval = newProjections.SelectProjection

        'DG Added:
        Return newProjections.SelectProjection()
...
#End Region
    ...
    Private Sub StatusBar1_PanelClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.StatusBarPanelClickEventArgs) Handles StatusBar1.PanelClick
        'StatusBarPanelScale
        If e.StatusBarPanel Is StatusBar1.Panels("StatusBarPanelScale") Then
            DoSetScale()
        End If
    End Sub
    ...
    Private Sub Legend_LayerChartClicked(ByVal Handle As Integer) Handles Legend.LayerChartClicked
        If (MapMain.ShapeDrawingMethod = MapWinGIS.tkShapeDrawingMethod.dmNewSymbology) Then
            frmMain.Plugins.BroadcastMessage("LAYER_EDIT_CHARTS" + Handle.ToString())
        End If
    End Sub
    ...
    Private Sub Legend_LayerChartFieldClicked(ByVal Handle As Integer, ByVal FieldIndex As Integer) Handles Legend.LayerChartFieldClicked
        If (MapMain.ShapeDrawingMethod = MapWinGIS.tkShapeDrawingMethod.dmNewSymbology) Then
            frmMain.Plugins.BroadcastMessage("LAYER_EDIT_CHARTS" + Handle.ToString() + "!" + FieldIndex.ToString())
        End If
    End Sub
    ...
    Private Sub MenuStrip1_LocationChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuStrip1.LocationChanged
