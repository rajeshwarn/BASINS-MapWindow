...
#Region "Initialization"
    ...
    Public Sub InitializeMapsAndLegends()
        ...
        If NeedDefaultConfig Then
            ...
            Catch e As Exception
                MapWinUtility.Logger.Dbg("Error setting dock windows: " + e.Message)
            End Try
        End If
    ...
    Public Function CreateMapPanel() As WeifenLuo.WinFormsUI.Docking.DockContent
        ...
        MapMain.MapResizeBehavior = MapWinGIS.tkResizeBehavior.rbKeepScale

        ' PM 18 Jan. 2012 - Make this setting off by default:
    ...
    Private Sub PanelsDefault()
        UpdateLegendPanel(True)
        If legendPanel IsNot Nothing Then legendPanel.DockTo(Me.dckPanel, DockStyle.Left)
        UpdatePreviewPanel(True)
        If previewPanel IsNot Nothing Then previewPanel.DockTo(Me.dckPanel, DockStyle.Left)
    End Sub
    ...
    Public Sub UpdateLegendPanel(ByVal LegendVisible As Boolean)
        If legendPanel IsNot Nothing Then
    ...
    Public Sub SetUpMenus()
        ...
        'Renamed back to File for BASINS: m_Menu.AddMenu("mnuFile", Nil, resources.GetString("mnuProject.Text"))
        ...
        m_Menu.AddMenu("mnuPanelsDefault", "mnuRestoreMenu", Nil, "Default Positions")
...
#Region "Public Properties"
    ...
    Public Function RecentFilesList() As String
        Dim lList As String = "Not Set"
        Dim lFileMenu As Windows.Forms.ToolStripMenuItem = Me.MenuStrip1.Items("mnuFile")
        If lFileMenu IsNot Nothing Then
            Dim lRecentMenu As Windows.Forms.ToolStripMenuItem = lFileMenu.DropDownItems("mnuRecentProjects")
            If lRecentMenu IsNot Nothing Then
                lList = lRecentMenu.Text & " - " & lRecentMenu.Name & vbCrLf
                For Each lSubMenu As Windows.Forms.ToolStripMenuItem In lRecentMenu.DropDownItems
                    lList &= lSubMenu.Text & " - " & lSubMenu.Name & vbCrLf
                Next
            End If
        End If
        Return lList
...
#Region "Legend Events"
    ...
    Private Sub Legend_GroupDoubleClick(ByVal Handle As Integer) Handles Legend.GroupDoubleClick
        ...
        'First see if the plug-ins want this event. If not then show the legend editor.
    ...
    Public Sub HandleClickedMenu(ByVal MenuName As String)
        ...
        If Not (m_PluginManager.ItemClicked(MenuName)) Then
            ...
            Select Case MenuName
                ...
                Case "mnuPanelsDefault" : PanelsDefault()
                Case "mnuNew" : DoNew()
                Case "mnuOpen" : DoOpen()
...
#Region "General Functions"
    ...
    Public Sub SetModified(ByVal Status As Boolean)
        ...
        MapMain.ShowVersionNumber = False ' AppInfo.ShowMapWindowVersion
        MapMain.ShowRedrawTime = False ' AppInfo.ShowHideRedrawSpeed
    ...
    Public Function GetProjectionFromUser(ByVal DialogCaption As String, ByVal DefaultProjection As String) As String Implements Interfaces.IMapWin.GetProjectionFromUser
        ...
        If Not String.IsNullOrEmpty(DialogCaption) Then form.Text = DialogCaption
        Dim lProj4 As String = ""
        If (form.ShowDialog() = Windows.Forms.DialogResult.OK) Then
            Dim lCoordinateSystem As MapWindow.Controls.Projections.CoordinateSystem = form.projectionTreeView1.SelectedCoordinateSystem
            If lCoordinateSystem IsNot Nothing Then
                lProj4 = lCoordinateSystem.proj4
                If String.IsNullOrEmpty(lProj4) Then
                    Dim proj As MapWinGIS.GeoProjection = New MapWinGIS.GeoProjection()
                    If proj.ImportFromEPSG(lCoordinateSystem.Code) Then
                        lProj4 = proj.ExportToProj4
                    End If
                End If
                If String.IsNullOrEmpty(lProj4) Then
                    MessageBox.Show("Proj4 not found for selected projection", modMain.AppInfo.Name, MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
            End If
        End If
        Return lProj4
...
#End Region
    ...
    Public Sub BuildRecentProjectsMenu()
        ...
        Try 'Use the direct MenuStrip1 reference to remove items from mnuRecentProjects that should have been removed above but were not
            Dim lFileMenu As Windows.Forms.ToolStripMenuItem = MenuStrip1.Items("mnuFile")
            If lFileMenu IsNot Nothing Then
                Dim lRecentMenu As Windows.Forms.ToolStripMenuItem = lFileMenu.DropDownItems("mnuRecentProjects")
                If lRecentMenu IsNot Nothing AndAlso lRecentMenu.DropDownItems IsNot Nothing Then
                    lRecentMenu.DropDownItems.Clear()
                End If
            End If
        Catch
        End Try
