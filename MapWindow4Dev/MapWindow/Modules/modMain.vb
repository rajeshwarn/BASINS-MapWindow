...
Module modMain
    ...
    Public Sub Main()
        ...
        If AppInfo.ShowWelcomeScreen = False _
           OrElse AppInfo.SplashTime = 0 _
           OrElse Reflection.Assembly.GetEntryAssembly.Location.EndsWith("USGSToolbox.exe") Then
            M_NOLOGO = True
        End If

        ' logo screen
        Dim formLogo As frmLogo = Nothing

        If Not M_NOLOGO Then
            formLogo = New frmLogo
            formLogo.Show()
            formLogo.lblVersion.Text = "MapWindow Version: " + App.VersionString
        ...
        For Each s As String In g_KillList
            MapWinUtility.modFile.TryDelete(s, True)
        Next
        g_KillList.Clear()
    ...
    Private Sub CheckPROJNAD()
        If System.IO.File.Exists(BinFolder & "\setenv.exe") AndAlso System.IO.Directory.Exists(BinFolder & "\PROJ_NAD") Then
            ...
            Dim lEnvTargets() As EnvironmentVariableTarget = {EnvironmentVariableTarget.Machine, EnvironmentVariableTarget.User, EnvironmentVariableTarget.Process}
            For Each lEnvTarget As EnvironmentVariableTarget In lEnvTargets
                Try
                    If Environment.GetEnvironmentVariable("PROJ_LIB", lEnvTarget).ToUpper <> BinFolder.ToUpper & "\PROJ_NAD" Then
                        Environment.SetEnvironmentVariable("PROJ_LIB", BinFolder & "\PROJ_NAD", lEnvTarget)
                    End If
                    If Environment.GetEnvironmentVariable("GDAL_DATA", lEnvTarget).ToUpper <> BinFolder.ToUpper & "\GDAL_DATA" Then
                        Environment.SetEnvironmentVariable("GDAL_DATA", BinFolder & "\GDAL_DATA", lEnvTarget)
                    End If
                Catch e As Exception
                    MapWinUtility.Logger.Dbg("Did not set PROJ_LIB/GDAL_DATA environment variable: " + e.ToString())
                End Try
            Next
        End If
    End Sub
    ...
    Private Sub LoadConfig()
        ...
        If IO.File.Exists(ProjInfo.ProjectFileName) Then
        ...
        If String.IsNullOrEmpty(ProjInfo.ConfigFileName) Then
    ...
    Public Function ReadLayerPropertyValueFromXML(ByVal XMLFileName As String, ByVal XMLRootName As String, ByVal XMLPropertyName As String, Optional ByVal DefaultValue As String = "") As String
