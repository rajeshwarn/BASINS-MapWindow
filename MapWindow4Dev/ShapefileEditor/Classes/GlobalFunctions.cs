    ...
    public class GlobalFunctions
    {
        ...
        private void UpdateUndoButtons()
        {
            try
            {
                ...
                MapWindow.Interfaces.ToolbarButton btn = MapWin.Toolbar.ButtonItem(c_UndoLastChangeButton);
                if (btn != null)
                {
                    btn.Enabled = (UndoStack.Count > 0);
                }

                // Ensure the disable/enable buttons are right, while we're here
                if (UndoStack.Count > 0)
                {
                    btn = MapWin.Toolbar.ButtonItem(c_UndoEnableButton);
                    if (btn != null)
                    {
                        btn.Enabled = false;
                    }
                    btn = MapWin.Toolbar.ButtonItem(c_UndoDisableButton);
                    if (btn != null)
                    {
                        btn.Enabled = true;
                    }
        ...
        public void UpdateCurrentLayer()
        {
            ...
            else
            {
                ...
                if (lt == MapWindow.Interfaces.eLayerType.LineShapefile || lt == MapWindow.Interfaces.eLayerType.PointShapefile || lt == MapWindow.Interfaces.eLayerType.PolygonShapefile)
                {
                    ...
                    {
                        System.IO.FileInfo LayerInfo = new System.IO.FileInfo(m_CurrentLayer.Filename);
                        //when a plugin has been created a new shapefile, during the creation there is no length, therefore, this line throws an error. Teva 10/04/2011                                  
                        //m_LayerSize = LayerInfo.Length;
                    }
                    // End modifications Paul Meems, 26 Oct. 2009
                }
