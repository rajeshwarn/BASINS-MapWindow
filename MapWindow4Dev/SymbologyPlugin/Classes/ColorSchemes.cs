...
namespace mwSymbology.Classes
{
    ...
    public class ColorSchemes
    {
        ...
        // The reference to the MapWindow
        ...
        public ColorSchemes(MapWindow.Interfaces.IMapWin mapWin, ColorSchemeType type)
        {
            _type = type;
            _mapWin = mapWin;
            if (_mapWin == null)
            {
                throw new Exception("ColorShemes class: reference to MapWin wasn't passed");
            }
        ...
        public void Serialize2Xml()
        {
            ...
            if (Directory.Exists(path))
            {
                ...
                {
                    SymbologyPlugin.MessageBoxError("Failed to save color schemes: " + path + Environment.NewLine + ex.Message);
                }
            }
        }
        ...
        public static MapWinGIS.ColorScheme ColorBlend2ColorScheme(ColorBlend blend)
        {
            ...
            {
                scheme.AddBreak((double)blend.Positions[i], MapWinUtility.Colors.ColorToUInteger(blend.Colors[i]));
            }
            return scheme;
        }
        ...
        public static ColorBlend ColorScheme2ColorBlend(MapWinGIS.ColorScheme scheme)
        {
            ...
            for (int i = 0; i < scheme.NumBreaks; i++)
            {
                ...
                blend.Colors[i] = MapWinUtility.Colors.IntegerToColor(color);
            }
            return blend;
        }
        ...
        public void SetDefaultColorScheme(Shapefile sf)
        {
            ...
            ShpfileType shpType = SymbologyPlugin.ShapefileType2D(sf.ShapefileType);
            if (shpType == ShpfileType.SHP_POINT || shpType == ShpfileType.SHP_POLYGON || shpType == ShpfileType.SHP_MULTIPOINT)
            {
                blend.Colors[0] = MapWinUtility.Colors.IntegerToColor(sf.DefaultDrawingOptions.FillColor);
                blend.Colors[1] = MapWinUtility.Colors.IntegerToColor(sf.DefaultDrawingOptions.FillColor);
            }
            else if (shpType == ShpfileType.SHP_POLYLINE)
            {
                blend.Colors[0] = MapWinUtility.Colors.IntegerToColor(sf.DefaultDrawingOptions.LineColor);
                blend.Colors[1] = MapWinUtility.Colors.IntegerToColor(sf.DefaultDrawingOptions.LineColor);
            }
        }
        ...
        {
            SetFirstColorScheme(MapWinUtility.Colors.IntegerToColor(color));
        }
