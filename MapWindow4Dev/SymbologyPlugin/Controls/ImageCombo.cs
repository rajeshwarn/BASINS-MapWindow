...
namespace System.Windows.Forms
{
    ...
    public class ImageCombo : ComboBox
    {
            ...
            for (int i = 0; i < _itemCount; i++)
            {
                switch (style)
                {
                    case ImageComboStyle.FrameType:
                    {
                        ...
                        else if (i == 1) str = "ftRounded";
                        else if (i == 2) str = "ftPointed";
                        break;
                    }
        ...
        public void RefreshImageList()
        {
            ...
            else if (m_style == ImageComboStyle.FrameType)
            {
                width = 36;
            }
            ...
            for (int i = 0; i < _itemCount; i++)
            {
                Bitmap img = new Bitmap(_imgWidth, _imgHeight, PixelFormat.Format32bppArgb);
                Graphics g = Graphics.FromImage(img);

                Color colorOutline = this.Enabled ? Color.Black : Color.Gray;
                ...
                switch (m_style)
                {
                    ...
                    case ImageComboStyle.FrameType:
                    {
                        if (i == 0 )
                        {
                            ...
                            g.DrawRectangle(new Pen(colorOutline), rect);
                        }
                        else if (i == 1)
                        {
                            g.FillEllipse(new SolidBrush(_color1), rect);
                            g.DrawEllipse(new Pen(colorOutline), rect);
                        }
                        else if  (i == 2)
                        {
                            float left = rect.X;
                            ...
                            g.DrawPath(new Pen(colorOutline), path);
                            path.Dispose();
                            break;
                        }
                        break;
                    ...
                    case ImageComboStyle.LineStyle: 
                    {
                        Color clr = this.Enabled ? _outlineColor : Color.Gray;
                        Pen pen = new Pen(clr);
                    ...
                    case ImageComboStyle.PointShape:
                    {
                        ...
                        sdo.FillColor = MapWinUtility.Colors.ColorToUInteger(this._color1);
                        sdo.LineColor = MapWinUtility.Colors.ColorToUInteger(this._outlineColor);
                        ...
                        IntPtr ptr = g.GetHdc();
                        sdo.DrawPoint(ptr, 0.0f, 0.0f, _imgWidth, _imgHeight, MapWinUtility.Colors.ColorToUInteger(this.BackColor));
                        g.ReleaseHdc(ptr);
                        break;
                    }
                    case ImageComboStyle.HatchStyle:
                        ...
                        if (_colorSchemes != null)
                        {
                            ...
                            if (blend != null)
                            {
                                ...
                                if (scheme != null)
                                {
                                    ...
                                    while (n < _imgWidth)
                                    {
                                        Color clr = MapWinUtility.Colors.IntegerToColor(scheme.get_RandomColor(rnd.NextDouble()));
            ...
            else
            {
                ...
                if (this.Items[e.Index].GetType() == typeof(ImageComboItem))
                {
                    ...
                    if (item.ImageIndex != -1 && item.ImageIndex < this.ImageList.Images.Count)
                    {
                        ...
                        // draw text (indented)
                        Color colorOutline = this.Enabled ? Color.Black : Color.Gray;
                        e.Graphics.DrawString(item.Text, font, new SolidBrush(colorOutline), e.Bounds.Left + m_list.ImageSize.Width + 3 /*offset*/, e.Bounds.Top);
                    }
                    else
                    {
