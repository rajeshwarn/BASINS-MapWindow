...
namespace mwSymbology.Controls
{
    ...
    /// </summary>
    [ToolboxItem(true)]
    public partial class LinePatternControl : ListControl
    {
        #region Declaration
        // The list of icons
        ...
        internal MapWinGIS.LinePattern SelectedPattern
        {
            get
            {
                if (this.ItemCount == 0 || this.SelectedIndex < 0 || this.SelectedIndex >= _patterns.Count)
                {
                    return null;
                }
                else
        ...
        public bool SaveToXML()
        {
            ...
            XmlElement xelSchemes = xmlDoc.CreateElement("LinePatterns");
        ...
        public void LoadFromXML()
        {
            ...
            if (System.IO.File.Exists(filename))
            {
                ...
                XmlElement xelSchemes = xmlDoc.DocumentElement["LinePatterns"];
                if (xelSchemes != null)
                {
                    foreach (XmlNode nodePattern in xelSchemes.ChildNodes)
                    {
                        MapWinGIS.LinePattern pattern = new MapWinGIS.LinePattern();
                        pattern.Deserialize(nodePattern.InnerText);
                        if (pattern.Count > 0)
                        {
                            _patterns.Add(pattern);
                        }
        ...
        void Control_OnDrawItem(Graphics graphics, RectangleF rect, int itemIndex, bool selected)
        {
            ...
            pattern.Draw(ptr, rect.X + 1.0f, rect.Y + 1.0f, (int)rect.Width - 2, (int)rect.Height - 2, MapWinUtility.Colors.ColorToUInteger(this.BackColor));
            graphics.ReleaseHdc();
        }
        #endregion
    }
