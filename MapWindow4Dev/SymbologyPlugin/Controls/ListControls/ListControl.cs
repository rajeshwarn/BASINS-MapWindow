...
namespace mwSymbology.Controls
{
    ...
    public partial class ListControl : UserControl
    {
        ...
        public ListControl()
        {
            ...
            Font fnt = new Font("Arial", (float)_cellSize.Height * 0.8f);
            this.Font = fnt;
        }
        ...
        private void RecalcLayout()
        {
            ...
            _colCount = (this.Width) / (int)_cellSize.Width;
            if (_colCount == 0)
            {
            ...
            if (_rowCount * _cellSize.Height <= this.Height)
            {
            ...
            {
                // scrollbar is needed
                _colCount = (this.Width - vScrollBar1.Width) / (int)_cellSize.Width;
                if (_colCount == 0)
                {
                    vScrollBar1.Visible = false;
                    return;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if (_resizeNeeded)
            {
                if (this.Width > 0 && this.Height > 0)
                {
                    ...
                    vScrollBar1.Maximum = _backbuffer.Height; //- this.Height + vScrollBar1.LargeChange;
        ...
        private void DrawGrid()
        {
            ...
            Color lineColor = this.Enabled ? _gridColor : Color.Gray;

            if (_gridVisible && _itemCount > 0)
            {
                for (int col = 0; col <= _colCount; col++)
                {
                    _graphics.DrawLine(new Pen(lineColor), new PointF(col * _cellSize.Width, 0), new PointF(col * _cellSize.Width, _rowCount * _cellSize.Height));
                }
                for (int row = 0; row <= _rowCount; row++)
                {
                    _graphics.DrawLine(new Pen(lineColor), new PointF(0, row * _cellSize.Height), new PointF(_colCount * _cellSize.Width, row * _cellSize.Height));
                }
            }
            DrawItems();
        }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            float column = (float)(e.X - this.ClientRectangle.X) / (float)_cellSize.Width;
            float row = (float)(e.Y - this.ClientRectangle.Y + vScrollBar1.Value) / (float)_cellSize.Height;
        ...
        protected override void OnMouseWheel(MouseEventArgs e)
        {
            ...
            {
                int StepSize;
                int MaxSize = vScrollBar1.Maximum - this.Height;
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (selectionChanged)
            {
                ...
                if (y2 > vScrollBar1.Value + this.Height)
                {
                    vScrollBar1.Value = y2 - this.Height + 1;
                }
