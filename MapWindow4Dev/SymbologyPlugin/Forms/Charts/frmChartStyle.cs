...
namespace mwSymbology.Forms
{
    ...
    public partial class frmChartStyle : Form
    {
        ...
        private SymbologyPlugin _plugin = null;
        private string _initState = "";
        private int _handle = -1;

        /// <summary>
        /// Initializes a new instance of the frmChartStyle class
        /// </summary>
        public frmChartStyle(SymbologyPlugin plugin, MapWinGIS.Shapefile sf, bool legendCall, int handle)
        {
            InitializeComponent();
            _shapefile = sf;
            _charts = sf.Charts;
            ...
            icbColors.ColorSchemes = _plugin.ChartColors;
        ...
        private void GUI2Settings(object sender, EventArgs e)
        {
            ...
            _charts.ValuesFontColor = MapWinUtility.Colors.ColorToUInteger(clpFont.Color);
            ...
            _charts.ValuesFrameColor = MapWinUtility.Colors.ColorToUInteger(clpFrame.Color);
        ...
        private void Draw()
        {
            ...
            IntPtr hdc = g.GetHdc();
            uint backColor = MapWinUtility.Colors.ColorToUInteger(this.BackColor);
            if (_charts.Visible)
            {
        ...
        private void Settings2GUI()
        {
            ...
            clpFont.Color = MapWinUtility.Colors.IntegerToColor(_charts.ValuesFontColor);
            chkFontItalic.Checked = _charts.ValuesFontItalic;
            udFontSize.SetValue(_charts.ValuesFontSize);

            clpFrame.Color = MapWinUtility.Colors.IntegerToColor(_charts.ValuesFrameColor);
        ...
        bool ApplyOptions()
        {
            if (_charts.NumFields == 0 )
            {
                if (_charts.Count == 0)
                {
                    //MessageBox.Show("No fields were chosen. No charts will be displayed.", _plugin.m_mapWin.ApplicationInfo.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //return false;
                }
                else
                {
                    if (MessageBox.Show("No fields were chosen. Do you want to remove all charts?",
                                    _plugin.m_mapWin.ApplicationInfo.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        _charts.Clear();
                    }
                    else
            ...
            else if (_shapefile.Charts.Count == 0)
            {
                ...
                ShpfileType type = SymbologyPlugin.ShapefileType2D(_shapefile.ShapefileType);
                if (type == ShpfileType.SHP_POINT || type == ShpfileType.SHP_MULTIPOINT)
                {
                    ...
                    Callback cback = new Callback(_plugin.m_mapWin);
                ...
                else
                {
                    ...
                    frmAddCharts form = new frmAddCharts(_shapefile, _plugin.m_mapWin);
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
           ...
           SymbologyPlugin.SaveLayerOptions(_plugin.m_mapWin, _handle);
       ...
       private void btnRefresh_Click(object sender, EventArgs e)
       {
        ...
        private void btnChartExpression_Click(object sender, EventArgs e)
        {
            ...
            frmQueryBuilder form = new frmQueryBuilder(_shapefile, _handle, s, false, _plugin.m_mapWin);
            if (form.ShowDialog() == DialogResult.OK)
            {
        ...
        {
            frmColorSchemes form = new frmColorSchemes(ref _plugin.ChartColors);
            if (form.ShowDialog(this) == DialogResult.OK)
            {
                _noEvents = true;
                this.icbColors.ColorSchemes = _plugin.ChartColors;
                if (_plugin.m_mainForm != null)
                {
                    _plugin.m_mainForm.UpdateColorSchemes();
                }
                _noEvents = false;
            }
            form.Dispose();
        ...
        {
            AxMapWinGIS.AxMap map = _plugin.m_mapWin.GetOCX as AxMapWinGIS.AxMap;
            if (map != null)
            {
