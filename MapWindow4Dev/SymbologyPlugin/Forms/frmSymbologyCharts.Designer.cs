...
namespace mwSymbology.Forms
{
    ...
    partial class frmSymbologyMain
    {
        ...
        private void DrawChartsPreview()
        {
            ...
            if (m_shapefile.Charts.Count > 0 && m_shapefile.Charts.NumFields > 0)
            {
                ...
                if (charts.ChartType == MapWinGIS.tkChartType.chtPieChart)
                    charts.DrawChart(ptr, (width - charts.IconWidth) / 2, (height - charts.IconHeight) / 2, false, MapWinUtility.Colors.ColorToUInteger(Color.White));
                else
                    charts.DrawChart(ptr, (width - charts.IconWidth) / 2, (height - charts.IconHeight) / 2, false, MapWinUtility.Colors.ColorToUInteger(Color.White));
        ...
        {
            frmChartStyle form = new frmChartStyle(m_plugin, m_shapefile, false, m_layerHandle);
            form.ShowDialog();
        ...
        {
            if (MessageBox.Show("Do you want to delete charts?", m_mapWin.ApplicationInfo.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
        ...
        {
            frmColorSchemes form = new frmColorSchemes(ref m_plugin.ChartColors);
