...
CheckCache:
    ...
    Public Function GetTileFilename(ByVal aTileFilename As String, _
        ...
        If IO.File.Exists(lActualFilename) Then
            'uncomment to enable replacement of stale tiles
    ...
    Public Overrides Sub ServiceItem(ByVal aQueueItem As clsQueueItem)
        Select Case aQueueItem.ItemType
            Case QueueItemType.TileItem
                ...
                ' Paul Meems: Download more tiles:
                'Dim zoomMax As Integer = aQueueItem.Zoom + 4
                'If zoomMax > g_TileServer.ZoomMax Then zoomMax = g_TileServer.ZoomMax
                'DownloadDescendants(aQueueItem.TilePoint, aQueueItem.Zoom, zoomMax, aQueueItem.ReplaceExisting)
    ...
    Friend Function DownloadFile(ByVal aUrl As String, ByVal aFileName As String, ByRef aActualFilename As String, ByVal aIsTile As Boolean, Optional ByVal aSuffix As String = Nothing) As Boolean
        ...
        Try
            If EnsureDirForFile(aFileName) Then
                ...
                Try
                    Dim lHeaders As Net.WebHeaderCollection = Nothing
                    If (aSuffix IsNot Nothing) AndAlso (aSuffix.Length > 0) Then
                        ...
                        If lETagStart > -1 Then
                            ...
                            lHeaders = New Net.WebHeaderCollection
                            lHeaders.Set("If-None-Match", """" & lCachedETag & """")
                        End If
                    End If

                    response = MapWinUtility.Net.GetWebResponse(aUrl, lHeaders)
                    ...
                    End If
                Catch ex As Exception
                    If ex.Message.IndexOf("(304)") > -1 Then
                        '(304) Not Modified is not an error, it means we don't need to download new file
                        lMoveTo &= pETagExt & lCachedETag
                        lSuccess = True
                        lSkipped = True
                    Else 'If ex.Message.IndexOf("cannot access the file") > -1 Then 'ex.Message.IndexOf("(404)") > -1
                        lError = True
                    End If
                End Try
AfterDownload:
