...
Public Class clsQueueManager
        ...
        Set(ByVal value As Boolean)
            If value Then
                ...
                If pQueueRunner Is Nothing Then
                    pQueueRunner = New Threading.Thread(AddressOf RunQueueBackground)
                    pQueueRunner.Start()
                End If
            Else
                pQueueQuit = True
    ...
    ''' </remarks>
    Private Sub RunQueueBackground()
        While Not pQueueQuit            
            If RunQueue() Then
                Threading.Thread.Sleep(10)
            Else 'Sleep longer if there is nothing to do
    ...
    ''' <summary>
    ''' Service the highest priority item in any queue
    ''' </summary>
    ''' <returns>True if an item was serviced, False if all queues were empty</returns>
    ''' <remarks>Only one thread should call RunQueue. If a background thread is doing RunQueueBackground then no other thread should call RunQueue</remarks>
    Public Function RunQueue() As Boolean
        ...
        For Each lQueue As Generic.Queue(Of clsQueueItem) In pQueues
            If lQueue.Count > 0 Then ' select next item in lowest queue with items                   
                ...
                pQueueMutex.WaitOne()
                Dim lFinishedQueue As Boolean = False 'Becomes true only the moment we Dequeue the last item
                If lQueue.Count > 0 AndAlso lQueueItem.Equals(lQueue.Peek) Then 'Only dequeue if next item is the one we just finished
                    lQueue.Dequeue()
                ...
                If lFinishedQueue Then
                    ...
                    Next
                End If
                Return True ' Skip checking lower priority queues
            End If
            lQueueIndex += 1
        Next
        pQueueMutex.ReleaseMutex()
