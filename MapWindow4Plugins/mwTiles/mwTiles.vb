...
#Region "Initialize and Terminate"
    ...
    Public Sub Initialize(ByVal MapWin As MapWindow.Interfaces.IMapWin, ByVal ParentHandle As Integer) Implements MapWindow.Interfaces.IPlugin.Initialize
        ...
        g_ShowStatusProgress = False 'GetAppSetting("ShowStatusProgress", Not g_MapWin.ApplicationInfo.ApplicationName.StartsWith("BASINS"))
...
#End Region
    ...
    Public Sub MapExtentsChanged() Implements MapWindow.Interfaces.IPlugin.MapExtentsChanged
        'If pIsValidProject AndAlso Me.Enabled Then
    ...
    Private Function AddTileToMap(ByVal aTilePoint As System.Drawing.Point, ByVal aZoom As Integer, ByVal aFilename As String) As Boolean
        Try
            If Enabled Then
                ...
                Dim lLocalFilename As String = TileFilename(aTilePoint, aZoom, False).Replace(g_TileCacheFolder, pProjectTileCacheFolder & SafeFilename(g_TileServer.Name.Replace(" ", "")) & g_PathChar)
                ...
                If Enabled Then
                    ...
                    If lImage.Open(lLocalFilename, MapWinGIS.ImageType.USE_FILE_EXTENSION, True) Then
                        Try
                            ...
                            End If
                            Dim lTileGroup As LegendControl.Group = g_MapWin.Layers.Groups.ItemByHandle(lTileGroupHandle)
                            Dim layerCount As Integer = lTileGroup.LayerCount
                            'Only add if we just created empty group or if the first layer is visible, otherwise user has turned off visibility of group
                            'If layerCount < 1 OrElse lTileGroup.LayersVisible Then
